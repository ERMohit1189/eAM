<%@ Page Title="" Language="C#" MasterPageFile="~/14/comman_root_manager.master" AutoEventWireup="true" CodeFile="MarksEntryIXtoXII.aspx.cs" Inherits="MarksEntryIXtoXII" %>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $(document).on('blur', 'input[type=text]', function () {
                ...
                var HYAE = 0, HYAEPrac = 0, Test1 = 0, Test2 = 0, Test3 = 0;
                var MaxHYAE = 0, MaxHYAEPrac = 0, MaxMarks1 = 0, MaxMarks2 = 0, MaxMarks3 = 0;
                ...
                if (!isNaN(parseFloat($(this).closest('tr').find('td:eq(9) input[type=text]').val()))) {
                    HYAEPrac = parseFloat($(this).closest('tr').find('td:eq(9) input[type=text]').val());
                }
                
                if (!isNaN(parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(4) input[type=text]").val())) && !Test1HasHide) {
                    MaxMarks1 = parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(4) input[type=text]").val());
                }
                if (!isNaN(parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(5) input[type=text]").val())) && !Test2HasHide) {
                    MaxMarks2 = parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(5) input[type=text]").val());
                }
                if (!isNaN(parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(6) input[type=text]").val())) && !Test3HasHide) {
                ...
                if (!isNaN(parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(9) input[type=text]").val()))) {
                    MaxHYAEPrac = parseFloat($("[id*=divList] table thead tr:eq(1) th:eq(9) input[type=text]").val());
                }
                ...
                aggregateHYAE = (MonthlyTestAY + HYAE + HYAEPrac);
                $(this).closest('tr').find('td:eq(7) span').html(MonthlyTestAY>0?MonthlyTestAY.toFixed(1):"");
                $(this).closest('tr').find('td:eq(10) span').html(aggregateHYAE>0?aggregateHYAE.toFixed(1):"");
        ...
        function LoadClass() {
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/ddlClassMarkEntryG3.aspx") %>',
                data: {},
                dataType: "json",
                async: true,
        ...
        function LoadStream() {
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/ddlStreamMarkEntry.aspx") %>',
        ...
        function LoadFillList() {
            ...
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/MarksEntryIXtoXIIServer.aspx") %>',
                data: {
                    'ClassId': ClassId,
                    'BranchId': BranchId,
                    'StreamId': StreamId,
        ...
        function SaveMarks() {
            ...
            MaxMarks += $("[id*=divList] table thead tr:eq(1) th:eq(9) input[type=text]").val() + "$";
            ...
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/SaveMarksEntryIXtoXII.aspx") %>',
                data: {
                    'ClassId': ClassId,
                    'BranchId': BranchId,
                    'SectionId': SectionId,
