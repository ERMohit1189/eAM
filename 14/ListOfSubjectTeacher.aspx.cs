...
public partial class ListOfSubjectTeacher : System.Web.UI.Page
{
    ...
    {
        sql = "select sctm.Id,sctm.EmpId,sctm.Ecode,sctm.EmpName,' ('+scm.SectionName+')' SectionName,cm.ClassName,case when bm.IsDisplay=1 then bm.BranchName else '' end BranchName, sctm.Medium,sm.SubjectName,pm.PaperName, cm.CidOrder, sctm.LoginName, format(sctm.RecordDate, 'dd-MMM-yyyy hh:mm:ss tt')RecordedDT from ICSESubjectTeacherAllotment sctm ";
        ...
        if (drpSubjectPaper.SelectedIndex != 0)
        {
            sql = sql + " and pm.id=" + drpSubjectPaper.SelectedValue + " ";
        }
