...
public partial class AcademicSessionAssesment : System.Web.UI.Page
{
    ...
    public void displayEmpInfo()
    {
        ...
        for (int i = 0; i < Grd.Rows.Count; i++)
        {
            ...
            TextBox txtAssessment = (TextBox)Grd.Rows[i].FindControl("txtAssessment");
            string qq2 = "select Assessment from ICSEAcademicSessionAssesment where Srno='" + lblSrNo.Text + "' and Term='" + ddlTerm.Text + "'";
            qq2 = qq2 + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "'";
            txtAssessment.Text = oo.ReturnTag(qq2, "Assessment");
        }
    }
        ...
        for (int i = 0; i < Grd.Rows.Count; i++)
        {
            ...
            if (txtAssessment.Text.Trim() != "")
            {
                try
                {
                    ...
                    cmd = new SqlCommand();
                    cmd.CommandText = "ICSEAcademicSessionAssesmentProc";
                    cmd.Connection = con;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@SrNo", lblSrNo.Text);
                    cmd.Parameters.AddWithValue("@Term", ddlTerm.SelectedValue);
                    cmd.Parameters.AddWithValue("@Assessment", txtAssessment.Text.Trim());
