using System;
using System.Web.UI;
using System.Web.UI.WebControls;
using System.Data;
using System.Data.SqlClient;

public partial class admin_BankBranchMaster : Page
{
    string sql = "", _sql="";
    Campus _oo = new Campus();
    private SqlConnection _con;
    protected void Page_Load(object sender, EventArgs e)
    {
        _con = new SqlConnection();
        _con = _oo.dbGet_connection();
        Campus camp = new Campus(); camp.LoadLoader(loader);
        if (!IsPostBack)
        {
            GetBankMaster();
            GetBankBranchMaster();
        }
    }

    private void GetBankMaster()
    {
        _sql = "Select BankName, id from AccBankMaster";
        _oo.FillDropDown_withValue(_sql, ddlBank, "BankName", "id");
        _oo.FillDropDown_withValue(_sql, ddlBank0, "BankName", "id");
        ddlBank.Items.Insert(0, new ListItem("<--Select-->", ">"));
    }
    
    private void Reset()
    {
        txtAddress.Text = string.Empty;
        txtAddress0.Text = string.Empty;
        txtBankBranchName.Text = string.Empty;
        txtBankBranchName0.Text = string.Empty;
        txtIFSC.Text = string.Empty;
        txtIFSC0.Text = string.Empty;
        txtPIN.Text = string.Empty;
        txtPIN0.Text = string.Empty;
        txtRemark.Text = string.Empty;
        txtRemark0.Text = string.Empty;

        ddlBank.SelectedIndex = 0;
        ddlBank0.SelectedIndex = 0;
    }

    private void GetBankBranchMaster()
    {
        _sql = "select bb.*, bb.id ids, BankName from AccBankBranchMaster bb inner join AccBankMaster bm on bm.ID=bb.BankId";
        var dt = _oo.Fetchdata(_sql);
        gvBankBranchList.DataSource = dt;
        gvBankBranchList.DataBind();
        for (int i = 0; i < gvBankBranchList.Rows.Count; i++)
        {
            Label BranchId = (Label)gvBankBranchList.Rows[i].FindControl("Label36");
            LinkButton lbtnEdit = (LinkButton)gvBankBranchList.Rows[i].FindControl("lbtnEdit");
            LinkButton lbtnDelete = (LinkButton)gvBankBranchList.Rows[i].FindControl("lbtnDelete");
            _sql = "SELECT BankBranchId FROM AccVendorBank WHERE BankBranchId='" + BranchId.Text + "'";
            if (_oo.Duplicate(_sql))
            {
                lbtnEdit.Text = "<i class='fa fa-lock'></i>";
                lbtnDelete.Text = "<i class='fa fa-lock'></i>";
                lbtnEdit.Enabled = false;
                lbtnDelete.Enabled = false;
            }
        }
    }

    protected void btnInsert_Click(object sender, EventArgs e)
    {
        _sql = "SELECT BankBranchName FROM AccBankBranchMaster WHERE BankBranchName='" + txtBankBranchName.Text.Trim() + "' and BankId=" + ddlBank.SelectedValue + "";
        if (_oo.Duplicate(_sql))
        {
            Campus camp = new Campus(); camp.msgbox(Page, msgbox, "Duplicate Bank Name!", "A");
        }
        else
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.CommandText = "AccBankBranchMasterProc";
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Connection = _con;
                cmd.Parameters.AddWithValue("@BankId", ddlBank.SelectedValue.Trim());
                cmd.Parameters.AddWithValue("@BankBranchName", txtBankBranchName.Text.Trim());
                cmd.Parameters.AddWithValue("@Address", txtAddress.Text.Trim());
                cmd.Parameters.AddWithValue("@IFSC", txtIFSC.Text.Trim());
                cmd.Parameters.AddWithValue("@PIN", txtPIN.Text.Trim());
                cmd.Parameters.AddWithValue("@Remark", txtRemark.Text.Trim());
                cmd.Parameters.AddWithValue("@IsActive", rblIsActive.SelectedValue);
                cmd.Parameters.AddWithValue("@LoginName", Session["LoginName"].ToString());
                cmd.Parameters.AddWithValue("@Action", "insert");
                try
                {
                    _con.Open();
                    cmd.ExecuteNonQuery();
                    _con.Close();
                    Campus camp = new Campus(); camp.msgbox(Page, msgbox, "Submitted successfully.", "S");
                    GetBankBranchMaster();
                    Reset();
                }
                catch (Exception ex)
                {
                }
            }
        }
    }

    protected void btnReset_Click(object sender, EventArgs e)
    {
        Reset();
    }

    protected void lbtnEdit_Click(object sender, EventArgs e)
    {
        LinkButton chk = (LinkButton)sender;
        Label lblId = (Label)chk.NamingContainer.FindControl("Label36");
        string ss = lblId.Text;
        lblID.Text = ss;

        Label BankId = (Label)chk.NamingContainer.FindControl("BankId");
        Label BankBranchName = (Label)chk.NamingContainer.FindControl("BankBranchName");
        Label Address = (Label)chk.NamingContainer.FindControl("Address");
        Label IFSC = (Label)chk.NamingContainer.FindControl("IFSC");
        Label PIN = (Label)chk.NamingContainer.FindControl("PIN");
        Label Remark = (Label)chk.NamingContainer.FindControl("Remark");
        Label IsActive = (Label)chk.NamingContainer.FindControl("IsActive");

        ddlBank0.SelectedValue = BankId.Text;
        txtBankBranchName0.Text = BankBranchName.Text;
        txtAddress0.Text = Address.Text;
        txtIFSC0.Text = IFSC.Text;
        txtPIN0.Text = PIN.Text;
        txtRemark0.Text = Remark.Text;
        rblIsActive0.SelectedValue = (IsActive.Text == "Active" ? "1" : "0");
        Panel1_ModalPopupExtender.Show();
    }

    protected void lbtnDelete_Click(object sender, EventArgs e)
    {
        btnNo.Focus();
        LinkButton chk = (LinkButton)sender;
        Label lblId = (Label)chk.NamingContainer.FindControl("Label37");
        string ss = lblId.Text;
        lblValue.Text = ss;
        mpeDelete.Show();
    }

    protected void btnYes_Click(object sender, EventArgs e)
    {
        DeleteRecord();
    }

    public void DeleteRecord()
    {
        _sql = "Delete from AccBankBranchMaster where Id=" + lblValue.Text + "";

        using (SqlCommand cmd = new SqlCommand())
        {
            cmd.CommandText = _sql;
            cmd.CommandType = CommandType.Text;
            cmd.Connection = _con;
            try
            {
                _con.Open();
                cmd.ExecuteNonQuery();
                _con.Close();
                Campus camp = new Campus(); camp.msgbox(Page, msgbox, "Deleted successfully.", "S");
                GetBankBranchMaster();
                Reset();
            }
            catch (Exception)
            {
            }
        }
    }

    protected void btnNo_Click(object sender, EventArgs e)
    {

    }

    protected void Button3_Click(object sender, EventArgs e)
    {
        _sql = "SELECT BankBranchName FROM AccBankBranchMaster WHERE BankBranchName='" + txtBankBranchName0.Text + "' and BankId=" + ddlBank0.SelectedValue + " and id<>" + lblID.Text + "";
        if (_oo.Duplicate(_sql))
        {
            Campus camp = new Campus(); camp.msgbox(Page, msgbox, "Duplicate Bank Name!", "A");
            Panel1_ModalPopupExtender.Show();
        }
        else
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                cmd.CommandText = "AccBankBranchMasterProc";
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Connection = _con;
                cmd.Parameters.AddWithValue("@Id", lblID.Text.Trim());
                cmd.Parameters.AddWithValue("@BankId", ddlBank0.SelectedValue.Trim());
                cmd.Parameters.AddWithValue("@BankBranchName", txtBankBranchName0.Text.Trim());
                cmd.Parameters.AddWithValue("@Address", txtAddress0.Text.Trim());
                cmd.Parameters.AddWithValue("@IFSC", txtIFSC0.Text.Trim());
                cmd.Parameters.AddWithValue("@PIN", txtPIN0.Text.Trim());
                cmd.Parameters.AddWithValue("@Remark", txtRemark0.Text.Trim());
                cmd.Parameters.AddWithValue("@IsActive", rblIsActive0.SelectedValue);
                cmd.Parameters.AddWithValue("@Action", "Update");
                try
                {
                    _con.Open();
                    cmd.ExecuteNonQuery();
                    _con.Close();
                    Campus camp = new Campus(); camp.msgbox(Page, msgbox, "Updatted successfully.", "S");
                    GetBankBranchMaster();
                    Reset();
                }
                catch (Exception ex)
                {
                }
            }
        }
    }

    protected void rblIsExam_SelectedIndexChanged(object sender, EventArgs e)
    {

    }

    
}