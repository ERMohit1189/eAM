...
public partial class DayBookEntryReport : System.Web.UI.Page
{
	string sql = "", _sql = "", Sql = "";
	...
	private void GetTodayReport()
	{
		GetOpeningBalance();
		string str = "";
		heading.Text = "Day Book Entry Report (from " + txtDate.Text + " to " + txtToDate.Text + ")";
		sql = "select *, format(RecordDate, 'dd-MMM-yyyy hh:mm:ss tt')RecordedDate from AccDayBook where convert(date, RecordDate) between convert(date, '" + txtDate.Text + "') and convert(date, '" + txtToDate.Text + "') and branchcode=" + Session["BranchCode"] + " ";
		if (ddlHeadType.SelectedIndex != 0)
		{
			str = str + " Head Type : " + ddlHeadType.SelectedItem.Text;
			sql += " and HeadType='" + ddlHeadType.SelectedItem.Text + "'";
		}
		if (ddlHead.SelectedIndex != 0)
		{
			str = str + " |  Head : " + ddlHead.SelectedItem.Text;
			sql += " and HeadName='" + ddlHead.SelectedItem.Text + "'";
		}
		if (ddlHeadCategory.SelectedIndex != 0)
		{
			str = str + " |  Head Category : " + ddlHeadCategory.SelectedItem.Text;
			sql += " and HeadCategory='" + ddlHeadCategory.SelectedItem.Text + "'";
		}
		if (ddlPaymentMode.SelectedIndex != 0 && ddlHeadType.SelectedIndex != 0)
		{
			str = str + " | Mode : " + ddlPaymentMode.SelectedItem.Text;
			sql += " and PaymentMode='" + ddlPaymentMode.SelectedItem.Text + "'";
		}
		if (ddlPaymentMode.SelectedIndex != 0 && ddlHeadType.SelectedIndex == 0)
		{
			str = str + " Mode : " + ddlPaymentMode.SelectedItem.Text;
        ...
        {
            sql += " and LoginName='" + DropDownList1.SelectedValue + "'";
        }
        sql += " order by id asc";
		if (ddlPaymentMode.SelectedIndex == 0 && ddlHeadType.SelectedIndex == 0)
		{
	...
	private void GetOpeningClosing(string dat)
	{
		...
		sql += " where convert(date, RecordDate)<convert(date, '" + dat + "') and BranchCode=" + Session["BranchCode"] + " ";
		sql += " order by id desc ";
		sql += " select top(1)  Balance from AccDayBook  ";
		sql += " where convert(date, RecordDate)=convert(date, @RecordDate) and BranchCode=" + Session["BranchCode"] + " ";
		sql += "  and PaymentMode='Cash' order by id desc ";
		...
		Sql = " select top(1)  Balance from AccDayBook  ";
		sql += " where convert(date, RecordDate)=convert(date, '" + dat + "') and BranchCode=" + Session["BranchCode"] + " and PaymentMode='Cash' ";
		sql += " order by id desc ";
		if (_oo.Duplicate(Sql))
		{
