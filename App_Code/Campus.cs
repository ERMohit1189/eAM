...
public class Campus : System.Web.UI.Page
{
    ...
    public Campus()
    {
        ...
        //Local------------------------------------------------------------------------------------------------------
        //  con.ConnectionString = "Data Source=WIN-RHU6NFQ18SA;Initial Catalog=RedRoseeAMdb;User Id=sa; Password=Lucknow@123;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;Connection Timeout=60;";
        // con.ConnectionString = "Data Source=WIN-RHU6NFQ18SA;Initial Catalog=RedRoseeAMdb;User Id=sa; Password=Lucknow@123;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;Connection Timeout=60;";
        //con.ConnectionString = "Data Source=DESKTOP-3G19J3N;Initial Catalog=MICDb;Integrated Security=True;Max Pool Size=50000;Pooling=True;Connection Timeout=60;";
        con.ConnectionString = "Data Source=ASUS\\SQLEXPRESS;Initial Catalog=RedRoseeAMdb;User Id=sa; Password=admin@1234;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;Connection Timeout=60;";
        // con.ConnectionString = "Data Source=DESKTOP-3G19J3N;Initial Catalog=SGSeAMdb;Integrated Security=True;Max Pool Size=50000;Pooling=True;Connection Timeout=60;";
        //con.ConnectionString = "Data Source=DESKTOP-3G19J3N;Initial Catalog=SGSNEWDB;Integrated Security=True;Max Pool Size=50000;Pooling=True;Connection Timeout=60;";
        //Local------------------------------------------------------------------------------------------------------

        //AnitaRaibarely------------------------------------------------------------------------------------------------------
        //con.ConnectionString = "Data Source=SERVER-PC\\SQLEXPRESS;Initial Catalog=eAMdb;User Id=sa; Password=Lucknow@123;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
        //AnitaRaibarely------------------------------------------------------------------------------------------------------

        //LockBharti------------------------------------------------------------------------------------------------------
        //con.ConnectionString = "Data Source=SERVER-PC;Initial Catalog=eAMdb;User Id=sa; Password=Lucknow@123;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
        //LockBharti------------------------------------------------------------------------------------------------------

        //eAMKanpur--------------------------------------------------------------------------------------------------------------
        //con.ConnectionString = "Data Source=ADVENTUS_58367\\SQLEXPRESS;Initial Catalog=eAMdb;User Id=sa; Password=CD!fjvSxLh64c@$;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
        //eAMKanpur--------------------------------------------------------------------------------------------------------------

        //JCBEST--------------------------------------------------------------------------------------------------------------
        //con.ConnectionString = "Data Source=ADVENTUS_C06236\\SQLEXPRESS;Initial Catalog=eAMdb;User Id=sa; Password=CnkU8A3vHRg$W4c$;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
        //JCBEST--------------------------------------------------------------------------------------------------------------

        //RedRoseRajajipuram--------------------------------------------------------------------------------------------------------------
        //con.ConnectionString = "Data Source=ADVENTUS1\\SQLEXPRESS;Initial Catalog=eAMdb-New;User Id=sa; Password=CVQLHdvyq2m8%4c$;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
        //RedRoseRajajipuram--------------------------------------------------------------------------------------------------------------

        //RedRoseGomtiNagar--------------------------------------------------------------------------------------------------------------
        //con.ConnectionString = "Data Source=WIN-NG3DS98ISE0;Initial Catalog=eAMdb;User Id=sa; Password=*d/?p[~>#pG/zuJ#;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
        //RedRoseGomtiNagar--------------------------------------------------------------------------------------------------------------
    ...
    public SqlConnection dbGet_connection()
    {
        ...
        //JCBEST--------------------------------------------------------------------------------------------------------------
        //old con.ConnectionString = "Data Source=WIN-EJIF9511KCJ;Initial Catalog=eAMdb;User Id=sa; Password=vikash@123;encrypt=true;trustservercertificate=true;Max Pool Size=50000;Pooling=True;";
    ...
    public string FindRecieptNo()
    {
        ...
        if (Duplicate(_sql2))
        {
            ...
            //xx = IdGeneration(recieptno, DateTime.Now.Ticks.ToString() + randomNumber);
        }
        return xx;
    }
    ...
    public DataTable GridFill1(string sql)
    {
        SqlConnection con = dbGet_connection(); // your connection method
    ...
    public string NumberToString(decimal amount)
    {
        string _sql = "Select top(1) CurrencyName from setting";
        string CurrencyName = ReturnTag(_sql, "CurrencyName");
        ...
        int paisa = (int)((amount - rupees) * 100);

        string rupeeWords = ConvertNumberToWords(rupees);
        string paisaWords = paisa > 0 ? ConvertNumberToWords(paisa) + " Paisa" : "";

        string fullWords = rupeeWords;

        if (paisa > 0)
            fullWords += " and " + paisaWords;

        fullWords += " " + CurrencyName + " Only.";

        return FirstCharToUpper(fullWords.ToLower());
    }
    ...
    public string NumberToString2(long number, string paisa)
    {
        if (number == 0)
        ...
        if ((number / 100000) > 0)
        {
            words += NumberToString2(number / 100000, "") + " Lakh ";
            number %= 100000;
        }

        if ((number / 1000) > 0)
        {
            words += NumberToString2(number / 1000, "") + " Thousand ";
            number %= 1000;
        }

        if ((number / 100) > 0)
        {
            words += NumberToString2(number / 100, "") + " Hundred ";
            number %= 100;
        }
        ...
        string paisaWords = "";
        if (!string.IsNullOrEmpty(paisa) && paisa != "00")
        {
            int paisaValue = int.Parse(paisa);
            if (paisaValue > 0)
            {
                paisaWords = " and ";
                if (paisaValue < 20)
                    paisaWords += unitsMap[paisaValue];
                else
                {
                    paisaWords += tensMap[paisaValue / 10];
                    if ((paisaValue % 10) > 0)
                        paisaWords += " " + unitsMap[paisaValue % 10];
                }
                paisaWords += " Paisa";
            }
        }

        return words.Trim() + " Rupees" + paisaWords + " Only.";
    }
    ...
    public void ExportControlToExcel(HttpResponse response, string fileName, Control control)
    {
        response.Clear();
