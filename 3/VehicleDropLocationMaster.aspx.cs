...
public partial class admin_VehicleDropLocationMaster : Page
{
    ...
    {
        sql = "Select VehicleNo,Id from VehicleDetails where VehicleType='" + DrpVehicleType0.SelectedItem.ToString() + "' and SessionName='" + Session["SessionName"].ToString() + "'";
    ...
    {
        sql = "Select vlm.Id,RouteName,DropPointName,DropDistance,ArrivalTime,DepartureTime,DisplayOrder,vm.VehicleType,vd.VehicleNo, DropPointId";
        sql = sql + " from VehicleDropLocationMaster vlm inner join VehicleRouteMaster vrm on vrm.Id=vlm.RouteId";
        ...
        sql = sql + " and vm.SessionName='" + Session["SessionName"].ToString() + "' and vd.SessionName='" + Session["SessionName"].ToString() + "'";
        ...
        {
            Label DropPointId = (Label)GridView1.Rows[i].FindControl("DropPointId");
            sql = "select LocationCode from LocationWiseVehicleAmount where LocationCode='" + DropPointId.Text + "' and SessionName='" + Session["SessionName"].ToString() + "' and BranchCode=" + Session["BranchCode"].ToString() + "";
            if (oo.Duplicate(sql))
            {
    ...
    protected void Insert()
    {
        ...
        {
            TextBox txtDropName = (TextBox)GridView2.Rows[j].FindControl("txtDropName");
            if (txtDropName.Text.Trim() == "")
            {
                Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Please, Enter Drop Stoppage Name!", "A");
                return;
            }
        }
        for (int i = 0; i < GridView2.Rows.Count; i++)
        {
            TextBox txtDropName = (TextBox)GridView2.Rows[i].FindControl("txtDropName");
            ...
            cmd = new SqlCommand();
            cmd.CommandText = "VehicleDropLocationMaster_Proc";
            ...
            cmd.Parameters.AddWithValue("@DropPointName", txtDropName.Text.Trim());
            cmd.Parameters.AddWithValue("@DropDistance", txtPickDistance.Text.Trim());
            ...
            cmd.Parameters.AddWithValue("@DropPointId", lblStopageCode.Text.Trim());
    ...
    protected void Update()
    {
        ...
        cmd.Parameters.AddWithValue("@DropPointName", txtDropStopageName.Text.Trim());
        cmd.Parameters.AddWithValue("@DropDistance", txtDropDistance.Text.Trim());
        cmd.Parameters.AddWithValue("@ArrivalTime", arrivalTime);
        cmd.Parameters.AddWithValue("@DepartureTime", departureTime);
        cmd.Parameters.AddWithValue("@DisplayOrder", txtDisplayOrder.Text.Trim());
        cmd.Parameters.AddWithValue("@DropPointId", "");
    ...
    protected void Delete()
    {
        ...
        cmd.Parameters.AddWithValue("@DropPointName", "");
        cmd.Parameters.AddWithValue("@DropDistance", "");
    ...
    public bool checkDuplicateLocationName()
    {
        ...
        {
            TextBox txtDropName = (TextBox)GridView2.Rows[i].FindControl("txtDropName");
            sql = "Select *from VehicleDropLocationMaster where ";
            ...
            sql = sql + " and DropPointName='" + txtDropName.Text.Trim() + "'";
            if (oo.Duplicate(sql))
            {
                txtDropName.Focus();
                txtDropName.BorderColor = System.Drawing.Color.Maroon;
                flag = true;
            }
        }
        return flag;
    ...
    protected void LinkButton2_Click(object sender, EventArgs e)
    {
        ...
        sql = "Select RouteId,DropPointName,DropDistance,Datepart(HH,ArrivalTime) as AHH,Datepart(MI,ArrivalTime) as AMM,DisplayOrder,";
        sql = sql + " Datepart(HH,DepartureTime) as DHH,Datepart(MI,DepartureTime) as DMM";
        sql = sql + " from VehicleDropLocationMaster where Id='" + lblID.Text + "' and SessionName='" + Session["SessionName"].ToString() + "' and BranchCode=" + Session["BranchCode"].ToString() + "";
        try
        {
        ...
        txtDropStopageName.Text = oo.ReturnTag(sql, "DropPointName");
        txtDropDistance.Text = oo.ReturnTag(sql, "DropDistance");
        ...
        loadVehicle0();
        sql = "Select VehicleTypeId from VehicleDropLocationMaster where Id='" + lblID.Text + "' and BranchCode=" + Session["BranchCode"].ToString() + " and SessionName='" + Session["SessionName"].ToString() + "'";
        DrpVehicleType0.SelectedValue = oo.ReturnTag(sql, "VehicleTypeId");
        loadVehicleNo0();
        sql = "Select VehicleNoid from VehicleDropLocationMaster where Id='" + lblID.Text + "' and BranchCode=" + Session["BranchCode"].ToString() + " and SessionName='" + Session["SessionName"].ToString() + "'";
    ...
    protected void LinkButton4_Click(object sender, EventArgs e)
    {
        ...
        sql = sql + " and VehicleNoid='" + Drpvehicleno.SelectedValue.ToString() + "' ";
        sql = sql + " and SessionName='" + Session["SessionName"].ToString() + "'";
        sql = sql + " and DropPointName='" + txtDropStopageName.Text.Trim() + "' and BranchCode=" + Session["BranchCode"].ToString() + "";
        if (!oo.Duplicate(sql))
        {
            if (DrpVehicleType0.SelectedIndex != 0 && Drpvehicleno0.SelectedIndex != 0 && txtDropStopageName.Text!="")
            {
                Update();
            }
            else
            {
                //oo.MessageBoxforUpdatePanel("Please Select VehicleType or VehicleNo.!", LinkButton1);
                Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Please Select VehicleType, VehicleNo & Drop Stoppage Name.!", "A");
    ...
    #region Method for setDropStopage and bindDrop grid
    protected void txtDropName_TextChanged(object sender, EventArgs e)
    {
        ...
        for (int i = 0; i < GridView2.Rows.Count; i++)
        {
            TextBox findtxtvalue = (TextBox)GridView2.Rows[i].FindControl("txtDropName");
    ...
    protected void txtDropStopage_TextChanged(object sender, EventArgs e)
    {
        bindStopageGrid();
    }
    public void bindStopageGrid()
    {
        ...
        bool flag;
        int stopage;
        flag = int.TryParse(txtDropStopage.Text.Trim(), out stopage);
        if (flag)
        {
            sql = "Select Max(DisplayOrder) as DisplayOrder from VehicleDropLocationMaster where ";
            sql = sql + " RouteId='" + DrpRouteName.SelectedValue.ToString() + "'";
            sql = sql + " and VehicleTypeId='" + DrpVehicleType.SelectedValue.ToString() + "'";
            sql = sql + " and VehicleNoid='" + Drpvehicleno.SelectedValue.ToString() + "' ";
    ...
    public void createId()
    {
        if (GridView2.Rows.Count > 0)
        {
            for (int i = 0; i < GridView2.Rows.Count; i++)
            {
                ...
                lblStopageCode.Text = "D" + DrpRouteName.SelectedValue + DrpVehicleType.SelectedValue + Drpvehicleno.SelectedValue + txtDisplayOrder.Text + oo.ReturnTag(sql, "DD") + oo.ReturnTag(sql, "MM") + oo.ReturnTag(sql, "Year") + oo.ReturnTag(sql, "Time");
            }
        }
    }  
    #endregion
    ...
    public static void updateGridViewReorder(int DisplayOrder, int id)
    {
        ...
        if (flag1 && flag2)
        {
            sql = "Update VehicleDropLocationMaster set DisplayOrder='" + DisplayOrder + "' where id='" + id + "' and BranchCode=" + BranchCode + "";
