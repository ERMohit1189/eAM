...
public partial class supplier_entry : System.Web.UI.Page
{
    ...
    {
        sql = "Select  ROW_NUMBER() OVER (ORDER BY id ASC) AS SrNo,id, SupplierCategory,SupplierName,Address,Country,State,City,Zip,Phone1,Phone2,Mobile,Fax,Email,Website,Remark,";
        sql = sql + "     SessionName,BranchCode,LoginName,RecordDate from SupplierInfoEntry ";
        ...
        for (int i = 0; i < GridView1.Rows.Count; i++)
        {
            ...
            sql = "SELECT Supplier FROM LibraryItemEntry WHERE Supplier='" + id.Text + "' AND BranchCode=" + Session["BranchCode"] + "";
            if (oo.Duplicate(sql))
            {
    ...
    {
        sql = "Select SupplierName from SupplierInfoEntry where SupplierName='" + txtName.Text.Trim().ToString() + "' and  BranchCode=" + Session["BranchCode"].ToString() + "";
        if (oo.Duplicate(sql))
        {
            Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Sorry, Duplicate Entry!", "A");
        }
        else if (drpSupplierCategory.SelectedItem.ToString() == "<--Select-->" || Drpcountry.SelectedItem.ToString() == "<--Select-->" || drpCity.SelectedItem.ToString() == "<--Select-->" || drpstate.SelectedItem.ToString() == "<--Select-->")
        {
        ...
        else
        {
            ...
            cmd.CommandText = "SupplierInfoEntryProce";
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Connection = con;
            cmd.Parameters.AddWithValue("@SupplierCategory", drpSupplierCategory.SelectedItem.ToString());
            cmd.Parameters.AddWithValue("@SupplierName", txtName.Text.Trim().ToString());
    ...
    protected void drpSupplierCategory_SelectedIndexChanged(object sender, EventArgs e)
    {
    }
    ...
    {
        sql = "Delete from SupplierInfoEntry where id=" + lblvalue.Text.Trim();
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = sql;
    ...
    {
        sql = "Select SupplierName from SupplierInfoEntry where SupplierName='" + txtName0.Text.Trim().ToString() + "' and id<>"+ lblID.Text.Trim() + " and  BranchCode=" + Session["BranchCode"].ToString() + "";
        if (oo.Duplicate(sql))
        {
            Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Sorry, Duplicate Entry!", "A");
            return;
        ...
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = "SupplierInfoEntryUpdateProce";
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Connection = con;
        cmd.Parameters.AddWithValue("@id", lblID.Text.Trim());
        cmd.Parameters.AddWithValue("@SupplierCategory", Label8.Text.Trim().ToString());
        cmd.Parameters.AddWithValue("@SupplierName", txtName0.Text.Trim().ToString());
    ...
    protected void LinkButton2_Click(object sender, EventArgs e)
    {
        ...
        lblID.Text = ss;
        sql = "Select  ROW_NUMBER() OVER (ORDER BY id ASC) AS SrNo,id, SupplierCategory,SupplierName,Address,Country,State as ss,City,Zip,Phone1,Phone2,Mobile,Fax,Email,Website,Remark,";
        sql = sql + "     SessionName,BranchCode,LoginName,RecordDate from SupplierInfoEntry ";
        sql = sql + "    where Id=" + ss + " and BranchCode=" + Session["BranchCode"].ToString() + "";
        Label8.Text = oo.ReturnTag(sql, "SupplierCategory");
        txtName0.Text = oo.ReturnTag(sql, "SupplierName");
