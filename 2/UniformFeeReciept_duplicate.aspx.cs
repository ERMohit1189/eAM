...
public partial class UniformFeeReciept_duplicate : Page
{
	...
	public void DisplayStudentCopy()
	{
		...
		sql += " Name as StudentName,asr.Srno,Receipt_no,FatherName,ClassName,SectionName,Gender, ofc.classid as classids,'OTHER FEE' HeadName,PaymentSatus,asr.ISDisplay,asr.BranchName,Mode,PaymentSatus Status,CancelledBy,FORMAT(CancelledDate,'dd MMM yyyy hh:mm:ss tt') CancelledDate";
		...
		sql += " Group by Receipt_no,Name,asr.Srno,FatherName,ClassName,SectionName,Gender,PaymentSatus,ISDisplay,BranchName,LoginName,DepositDate, NextDeuAmt, ofc.classid,Mode,PaymentSatus,CancelledBy,CancelledDate";
        ...
        if (lblStatus.Text == "Cancelled")
        {
            ...
            Label6.Text = oo.ReturnTag(sql, "CancelledBy");
		...
		sql = "Select Top 1 Format(DepositDate,'dd MMM yyyy hh:mm:ss tt') DepositDate, PaidAmt, LoginName  from UniformFeeDeposit where Receipt_no='" + Label1.Text + "'";
		lblFooterDate0.Text = lblFooterDate.Text = oo.ReturnTag(sql, "DepositDate");
        //DateTime.Parse(oo.ReturnTag(sql, "DepositDate")).ToString("dd MMM yyyy hh:mm:ss tt");
