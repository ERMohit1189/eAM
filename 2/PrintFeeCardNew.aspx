<%@ Page Title="" Language="C#" MasterPageFile="~/11/comman_root_manager.master" AutoEventWireup="true" CodeFile="PrintFeeCardNew.aspx.cs" Inherits="PrintFeeCardNew" %>
    ...
    <script>
        ...
        function loadSrno() {
            $.ajax({
                ...
                data: { 'classId': $("#drpclass").val(), 'SectionId': $("#drpsection").val(), 'BranchId': $("#drpBranch").val(), 'Status': $("#drpStatus").val() },
                dataType: "json",
                async: true,
                success: function (result) {
                },
        ...
        function PrintDiv() {
            ...
            printWindow.document.write('<html><head><title>Fee Card_' + $("#drpclass option:selected").text() + "(" + $("#drpsection option:selected").text() + ')</title>' + headContent + '</head>');
        ...
        function LoadData() {
            ...
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/PrintfeeCardServerNew.aspx") %>',
                data: {
                    'session': session,
                    'SrNo': SrNo,
                    'Section': Section,
