...
public partial class admin_TCCollectionReport : Page
{
...
#pragma warning restore 414
    ...
    protected void loadData()
    {
        ...
        sql = " select Row_Number() over (order by tc.Id Asc) as SNo ,tc.id,tc.srno,RecieptNo,format(TCIssueDate,'dd-MMM-yyyy') as TCIssueDate,format(AdmissionFromDate,'dd-MMM-yyyy') as AdmissionFromDate,StudentName,tc.FatherName,asd.ClassName Class,LoginName,tc.BranchCode,convert(nvarchar,RecordDate,106) as RecordDate,Sex,tc.FatherContactNo,ReceivedAmount,case when cancel IS NULL then 'No' else 'Yes' end as Cancel, mop, status, tcType  from TCCollection tc " +
            " Inner join AllStudentRecord_UDF('" + Session["SessionName"] + "','" + Session["BranchCode"] + "') asd on asd.SrNo=tc.srno";
        sql = sql + " where convert(date, RecordDate) between '" + fromDate + "' and '" + ToDate + "'";
        sql = sql + " and tc.BranchCode=" + ddlBranch.SelectedValue + " ";
        if (studentId != "")
        {
            sql = sql + " and tc.srno='" + studentId + "' ";
        }
        string sts = "";
        if (DdlpaymentMode.SelectedIndex != 0)
        {
            sts = sts + "tc.Mode : " + DdlpaymentMode.SelectedValue;
            sql = sql + " and tc.mop='" + DdlpaymentMode.SelectedValue + "' ";
        }
        if (drpStatus.SelectedIndex != 0)
        {
            sql = sql + " and tc.Status='" + drpStatus.SelectedValue + "' ";
            if (DdlpaymentMode.SelectedIndex != 0)
            {
                sts = sts + " | tc.Status : " + drpStatus.SelectedValue;
            }
            else
            {
                sts = sts + "tc.Status : " + drpStatus.SelectedValue;
        ...
        {
            sql = sql + " and tc.LoginName='" + DropDownList1.SelectedValue + "' ";
            sts = sts + " by " + DropDownList1.SelectedValue;
        }
        sql = sql + "  order by tc.id desc";
        lbloptions.Text = sts;
    ...
    protected void ImageButton1_Click(object sender, EventArgs e)
    {
        if (Grd.Rows.Count > 0)
        {
            oo.ExportToWord(Response, "TCCollection.doc", divExport);
        }
    }
    protected void ImageButton2_Click(object sender, EventArgs e)
    {
        if (Grd.Rows.Count > 0)
        {
            oo.ExportToExcel("TCCollection.xls", Grd);
        }
    }
