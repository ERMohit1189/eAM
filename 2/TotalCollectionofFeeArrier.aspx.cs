...
public partial class admin_TotalCollectionofFee : Page
{
    ...
    protected void LinkButton5_Click(object sender, EventArgs e)
    {
        ...
        sql = "  select max(cm.ClassName) as Class,ISNULL((select SUM(PaidAmount) from FeeReceiptWithInstallment frwi inner join FeeDeposite fd1 on fd1.RecieptSrNo = frwi.ReceiptNo and fd1.SessionName = frwi.SessionName where FeeDepositeDate between  '" + fromdate + "'   and   '" + todate + "' and fd1.Class = cm.ClassName and Cancel is null and fd1.MoP = 'Cash' and InstallmentName = 'Arrear'),0)+    ISNULL((Select RecievedAmount from FeeDeposite where FeeMonth = 'Arrier' and Class = cm.ClassName and Cancel is null      and FeeDepositeDate between '" + fromdate + "'   and   '" + todate + "'  and MoP = 'Cash'),0) ArrierAmount";

        sql = sql + ",sum(fd.RecievedAmount) as RecievedAmount,CIDOrder,fd.SessionName from ClassMaster cm";
        ...
        try
        {
            ...
            Label lbhh = (Label)GridView1.FooterRow.FindControl("Label260");
            lbhh.Text = oo.ReturnTag(sql, "ArrierTotAmt");
        }
        catch (Exception) { }
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            Label Salary = (Label)e.Row.FindControl("lblArriemAmount");
            m = m + Convert.ToDouble(Salary.Text);
        }
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            Label lblTotalPrice = (Label)e.Row.FindControl("Label26011");
            lblTotalPrice.Text = String.Format("{0:0.00}", m);// m.ToString();
        }
    }
}