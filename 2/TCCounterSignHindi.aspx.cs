...
public partial class _2_TCCounterSignHindi : System.Web.UI.Page
{
...
ORDER BY Id DESC";
    ...
    WHERE SrNo = @SrNo";
        ...
        sql = "Select Religion,FatherOccupation,G1Address,Name StudentName,FatherName,MotherName,Nationality,Category,convert(nvarchar,DOB,106) as DOB,DatePart(dd,DOB) as date,DateName(mm,DOB) as month,DatePart(yyyy,DOB) as year,RIGHT('0' + CAST(DAY(DOB) AS VARCHAR), 2) + '-' + RIGHT('0' + CAST(MONTH(DOB) AS VARCHAR), 2) + '-' + CAST(YEAR(DOB) AS VARCHAR) AS FormattedDOB,StLocalStateName,StLocalCityName,ApaarId from AllStudentRecord_UDF('" + oldsessionname + "'," + Session["BranchCode"] + ") where SrNo='" + Label31.Text + "'";
        ...
        Literal1.Text = oo.ReturnTag(sql, "ApaarID");
    ...
    protected void LinkButton2_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(DateofAdmission))
            {
                ...
                //DateTime dt;
                //if (DateTime.TryParseExact(DateofAdmission, "dd-MM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out dt))
                //{
                //    TextBox3.Text = dt.ToString("dd-MMM-yyyy");
                //}
                //else
                //{
                //    TextBox3.Text = ""; // fallback to empty if parsing fails
                //}
            }
            else
            {
            ...
            if (!string.IsNullOrWhiteSpace(DateofPromotion))
            {
                ...
                //DateTime dt;
                //if (DateTime.TryParseExact(DateofPromotion, "dd-MM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out dt))
                //{
                //    TextBox4.Text = dt.ToString("dd-MMM-yyyy");
                //}
                //else
                //{
                //    TextBox4.Text = ""; // fallback to empty if parsing fails
                //}
            }
            else
            {
