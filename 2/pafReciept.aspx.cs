    ...
    public partial class AdminPafReciept : Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (string.IsNullOrEmpty(Request.QueryString["rid"]))
                {
                    DisplayStudentCopy(Request.QueryString["AdmissionRecieptNo"].Replace("__", "/"));
        ...
        public void DisplayStudentCopy(string admissionRecieptNo)
        {
            ...
            _sql += " afc.LoginName,afc.BranchCode,Sex, cm.id Classid, case when isnull(afc.CheckDDNo, '') in ('','na', 'N/A') then '' else ' ('+isnull(afc.CheckDDNo, '')+')' end chequeNo, ";
            _sql += " afc.FatherContactNo,Amount Amount,bm.BranchName,afc.RecordDate,afc.AdmissionType,afc.LoginName,afc.status,afc.SessionName  from AdmissionFormCollection afc";
            ...
            _sql += " where RecieptNo='" + admissionRecieptNo + "' and afc.BranchCode=" + Session["BranchCode"];// + " and afc.SessionName='" + Session["SessionName"] + "'";
            ...
            if ((string)Session["txnchk"] == _oo.ReturnTag(_sql, "LoginName"))
            {
                ...
                ViewState["txnid"] = _oo.ReturnTag(_sql, "LoginName");
                ...
                //Label24.Text = _oo.ReturnTag(_sql, "CancelledBy").ToString();

                //Label155.Text = _oo.ReturnTag(_sql, "CancelledDate").ToString();
                //Label26.Text = _oo.ReturnTag(_sql, "CancelledDate").ToString();
