...
public partial class UniformReceiptCancellation : Page
{
    ...
    string sessionname = "";
    public UniformReceiptCancellation()
    {
    ...
    public void loadStudentGrid()
    {
        ...
        _sql = "select count(*) count from AllStudentRecord_UDF('" + sessionname + "', 1) where  Srno='" + studentId + "'";
        if (_oo.ReturnTag(_sql, "count") == "0")
        {
            _sql = "select top(1) SessionName from StudentOfficialDetails where Srno='" + studentId + "' order by id desc  ";
        ...
        _sql = _sql + "   BranchName, FamilyContactNo,PhotoPath from AllStudentRecord_UDF('" + sessionname + "', 1)";
    ...
    public void loadfee()
    {
        ...
        _sql = "select id, DepositDate, Receipt_no as ReceiptNo, PaymentSatus as Status, PaidAmt as Paid, NextDeuAmt NextDue from UniformFeeDeposit where SrNo=" + "'" + studentId + "'";
        _sql = _sql + " and PaymentSatus='Paid' and SessionName='" + Session["SessionName"].ToString() + "' and BranchCode=" + Session["BranchCode"].ToString() + "";
    ...
    {
        _sql = "update UniformFeeDeposit set PaymentSatus='Canceled' where Receipt_no='" + lblvalue.Text + "' and sessionname='"+Session["SessionName"]+"'";
