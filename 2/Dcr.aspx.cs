...
public partial class Dcr : Page
{
    ...
    protected void loaddata()
    {
        ...
        string Statuss = drpStatus.SelectedValue;
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                if (Statuss != "All")
                {
                    cmd.Parameters.AddWithValue("@Statuss", Statuss.Trim());
                }
                ...
                {
                    cmd.Parameters.AddWithValue("@LoginName", DropDownList1.SelectedValue);
                }
                ...
                if (dt.Rows.Count > 0)
                {
                    string ss1s = "select format(getdate(), 'dd-MMM-yyyy hh:mm:ss tt') genratedOn";
                    string ss2s = "select name+' ('+UserId+')' GeneatedBy from NewAdminInformation where UserId='" + Session["LoginName"] + "' and BranchCode=" + ddlBranch.SelectedValue + "";
                    generatedBy.Text = "Generated by " + oo.ReturnTag(ss2s, "GeneatedBy") + " on " + oo.ReturnTag(ss1s, "genratedOn");
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@SessionName", dt.Rows[i]["SessionName"].ToString());
                    ...
                    table += "<table id='tblData' class='table table-striped table-hover no-bm no-head-border table-bordered pro-table table-header-group'><tbody>";
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        if (TDataHead.Rows.Count > 0)
                        {
                            ...
                            if (DrpSessionName.SelectedIndex == 0 && drpStatus.SelectedIndex != 0)
                            {
                            ...
                            {
                                if (DdlpaymentMode.SelectedIndex != 0 || drpStatus.SelectedIndex != 0 || DrpSessionName.SelectedIndex != 0)
                                {
                            ...
                            table += "<tr>";
                            table += "<td class='text-left'>" + (i + 1) + "</td><td class='text-left'>" + dt.Rows[i]["ReceiptNo"].ToString() + "</td><td class='text-left'>" + dt.Rows[i]["Status"].ToString() + "</td><td class='text-left'>" + dt.Rows[i]["Mode"].ToString() + "</td>";
                            ...
                            table += "<td class='text-left'>" + Months + "</td>";
                            table += "<td class='text-left'>" + srno + "</td><td class='text-left'>" + dt.Rows[i]["Name"].ToString() + "</td><td class='text-left'>" + dt.Rows[i]["CombineClassName"].ToString() + "</td>";
                            ...
                            if (ds.Tables[2].Rows.Count > 0)
                            {
                                ...
                                double paid = 0;
                                if (dt.Rows[i]["Status"].ToString() != "Cancelled")
                                {
                                    paid = double.Parse(ds.Tables[2].Rows[0]["Paid"].ToString());
                                    Gtotal = Gtotal + double.Parse(ds.Tables[2].Rows[0]["Paid"].ToString());
                                }
                                table += "<td class='text-right'>" + paid.ToString("0.00") + "</td>";
                            }
                            else
                            {
        ...
        //Page.ClientScript.RegisterStartupScript(this.GetType(), "CallMyFunction", "getTotal()", true);
        //ClientScript.RegisterStartupScript(GetType(), "hwa", "alert('Hello World');", true);
        ScriptManager.RegisterClientScriptBlock(this.Page, this.GetType(), "MyFun1", "getTotal();", true);
    }
