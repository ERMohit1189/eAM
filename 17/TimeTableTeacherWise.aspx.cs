...
public partial class TimeTableTeacherWise : Page
{
    string sql = "", _sql = "", Sql = "";
    ...
    protected void LoadData()
    {
        ...
        for (int i = 0; i < DTClass.Rows.Count; i++)
        {
            ...
            for (int j = 0; j < DTBranchn.Rows.Count; j++)
            {
                ...
                {
                    sql = "select PaperCode from TTGenerateTimeTable where convert(date, RecordedDate)=convert(date, '" + txtDate.Text.Trim() + "') and classid=" + DTClass.Rows[i]["id"].ToString() + " and BranchId=" + DTBranchn.Rows[j]["id"].ToString() + " ";
                    sql +=  " and sectionId=" + DTSection.Rows[k]["id"].ToString() + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "' ";
                    sql +=  " and empCode='" + Session["LoginName"] + "' ";
                    sql +=  "  order by ClassId, BranchId, Sectionid asc";
                    if (_oo.Duplicate(sql))
                    {
                        ...
                        for (int l = 1; l < table.Columns.Count; l++)
                        {
                            ...
                            sql +=  " and convert(date, RecordedDate)=convert(date, '" + txtDate.Text.Trim() + "') and classid=" + DTClass.Rows[i]["id"].ToString() + " and BranchId=" + DTBranchn.Rows[j]["id"].ToString() + " ";
                            sql +=  " and sectionId=" + DTSection.Rows[k]["id"].ToString() + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "' ";
                            sql +=  " and empCode='" + Session["LoginName"] + "' ";
                            sql +=  "  order by ClassId, BranchId, Sectionid asc";
