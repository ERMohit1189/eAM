...
public partial class common_G2_ddlClass_MarkEntry_2 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["Logintype"].ToString() == "Admin")
        {
            ...
            _sql +=  " where cm.SessionName='" + Session["SessionName"] + "' and cm.BranchCode=" + Session["BranchCode"] + " and t1.SessionName='" + Session["SessionName"] + "' and t1.BranchCode=" + Session["BranchCode"] + " and GroupId='G2' Order by CIDOrder";
        ...
        else
        {
            ...
            _sql += " where T1.BranchCode=" + Session["BranchCode"] + " and t2.SessionName='" + Session["SessionName"] + "' and GroupId = 'G2' and ECode = '" + Session["LoginName"].ToString() + "'";
            _dt = DAL.DALInstance.GetValueInTable(_sql);
            if (_dt != null && _dt.Rows.Count > 0)
            {
                for (int i = 0; i < _dt.Rows.Count; i++)
