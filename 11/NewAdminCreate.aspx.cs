...
public partial class admin_NewUserCreate : System.Web.UI.Page
{
    ...
    public void ComposeSMS()
    {
        ...
        try
        {
            List<SqlParameter> param = new List<SqlParameter>()
            {
                ...
                new SqlParameter("@UserId",txtUserId.Text.ToString().Replace(" ", ""))

            };
            DataSet ds = oo.ReturnDataSet("USP_AdminSignUpTemplate", param.ToArray());
    ...
    public string SendSms(DataSet ds)
    {
        ...
        try
        {
            ...
            new SMS().SendSms(contactNo, msg, "13", Session["BranchCode"].ToString());
        ...
        if (oo.ReturnTag(sql, "HitValue") != "")
        {
            if (oo.ReturnTag(sql, "HitValue") == "true")
            {
                ...
                if (FmobileNo != "")
                {
                    ...
                    sql = "Select SmsSent From SmsEmailMaster where Id='13' and BranchCode=\" + Session[\"BranchCode\"] + \"";
                    if (oo.ReturnTag(sql, "SmsSent").Trim() == "true")
                    {
                        sms_response = sadpNew.Send(mess, FmobileNo, "13");
                    }
                }
            }
        }
