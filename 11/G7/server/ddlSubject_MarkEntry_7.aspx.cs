...
public partial class common_G7_ddlSubject_MarkEntry_7 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Session["Logintype"].ToString() == "Admin" || Convert.ToInt16(Session["count"].ToString()) > 0)
        {
            ...
            _sql +=  " where sgm.Id='" + drpSubjectGroup.ToString() + "' and sgm.BranchCode=" + Session["BranchCode"] + " and spm.BranchCode=" + Session["BranchCode"] + " and sm.BranchCode=" + Session["BranchCode"] + " and sm.SessionName='" + Session["SessionName"].ToString() + "' and (IsForExam=1 or IsForExam is null)  order by sm.id";
        ...
        else
        {
            ...
            _sql +=  " where Ecode='" + Session["LoginName"].ToString() + "' and spm.BranchCode=" + Session["BranchCode"] + " and sm.BranchCode=" + Session["BranchCode"] + " and sctm.BranchCode=" + Session["BranchCode"] + " and sctm.ClassId='" + drpclass.ToString() + "' ";
            _sql +=  " and (sctm.Branchid='" + drpBranch.ToString() + "' or sctm.Branchid is null) and sctm.SectionName='" + drpsectionName.ToString() + "' and sctm.SessionName='" + Session["SessionName"].ToString() + "'";
