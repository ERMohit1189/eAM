...
public partial class common_G6_MarksEntryXI_1718Server : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                sql +=  " where  sg.ClassName=" + ClassName.ToString() + " and sg.BranchId = '" + BranchId + "' ";
                ...
                if (dt.Rows.Count > 0)
                {
                    try
                    {
                        string MaxMarks1 = "", MaxMarks2 = "", MaxMarks3 = "", MaxMarks4 = "", MaxMarks5 = "", MaxMarks6 = "", MaxMarks7 = "", commanMax = "";
                        double MaxMarks1M = 0, MaxMarks2M = 0, MaxMarks3M = 0, MaxMarks4M = 0, MaxMarks5M = 0, MaxMarks6M = 0, MaxMarks7M = 0;
                        string sql1 = "Select MaxMarks1,MaxMarks2,MaxMarks3,MaxMarks4,MaxMarks5,MaxMarks6, MaxMarks7 from SetMaxMinMarks_XII where Eval='" + Evail.ToString().Trim() + "' and SubjectActivityId='" + SubjectId.ToString().Trim() + "' and SessionName='" + Session.ToString() + "' and SectionName='" + SectionName.ToString() + "' and BranchCode=" + BranchCode.ToString() + "";
                        string sql2 = "Select MaxMarks from SubjectMaster where Id='" + SubjectId.ToString().Trim() + "' and BranchCode=" + BranchCode.ToString() + " and SessionName='" + Session.ToString() + "' and SectionName='" + SectionName.ToString() + "'";
                        ...
                        Response.Write("<th class='p-tot-tit p-pad-n tab-b-15  tab-in' scope='col'><span id=''> N.B.</span><br><input name='' type='text' value='" + (MaxMarks4 == "" ? "50" : MaxMarks4).ToString() + "' id='MaxMarks4' class='form-control-blue text-center' style='width:40px;'></th>");
                        Response.Write("<th class='p-tot-tit p-pad-n tab-b-15  tab-in' scope='col'><span id=''> S.E.</span><br><input name='' type='text' value='" + (MaxMarks5 == "" ? "50" : MaxMarks5).ToString() + "' id='MaxMarks5' class='form-control-blue text-center' style='width:40px;'></th>");
                        Response.Write("<th class='p-tot-tit p-pad-n tab-b-15  tab-in' scope='col'><span id=''> H.Y.</span><br><input name='' type='text' value='" + (MaxMarks6 == "" ? "50" : MaxMarks6).ToString() + "' id='MaxMarks6' class='form-control-blue text-center' style='width:40px;'></th>");
                        Response.Write("<th class='p-tot-tit p-pad-n sub-m-w-70' scope='col'><span id=''> TOTAL </span><br><span id=''>100</span></th>");
                        ...
                        {
                            string Test1 = "", Test2 = "", Test3 = "", NB = "", SE = "", SAT = "", id = "", Prac="";
                            double Test1M = 0, Test2M = 0, Test3M = 0, NBM = 0, SEM = 0, SATM = 0, PracM = 0;
                            string sql3 = "Select Id,Test1,Test2,Test3,SAT,NB,SE, Prac from CCEXII_1718 where SRNO='" + dt.Rows[i]["srno"].ToString() + "' and Evaluation='" + Evail.ToString().Trim() + "' and SubjectId='" + SubjectId.ToString().Trim() + "' and SessionName='" + Session.ToString() + "' and BranchCode=" + BranchCode.ToString() + "";
                            ...
                            double totalmmmarks1 = p1 > p2 ? (p1 > p3 ? maxsum1 : (p2 > p3 ? maxsum2 : maxsum3)) : (p2 > p3 ? maxsum2 : maxsum3);
                            ...
                            double totals = (percentle1 + NBM + SEM + SATM);
                            Response.Write("<td class='p-pad-n text-center tab-in'><span id=''>" + totals.ToString("0.00") + "</span></td>");
                            double pTotal = 0;
                            if (totals > 0)
                            {
                                pTotal = pTotal + 10;
                            }
                            double bordPercentile = (totals * MaxMarks6M) / (MaxMarks4M + MaxMarks5M + MaxMarks6M + pTotal);
                            Response.Write("<td class='p-pad-n text-center tab-in'><span id=''>" + bordPercentile.ToString("0.00") + "</span></td>");
                            Response.Write("<td class='p-pad-n text-center tab-in'><input type='text' value='" + PracM.ToString() + "' class='form-control-blue text-center' name='7' style='width:40px;'></td>");
