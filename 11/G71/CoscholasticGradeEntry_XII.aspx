<%@ Page Title="" Language="C#" MasterPageFile="~/11/comman_root_manager.master" AutoEventWireup="true" CodeFile="CoscholasticGradeEntry_XII.aspx.cs" Inherits="staff_CoscholasticGradeEntry_XII" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderMainBox" runat="Server">
    ...
    <script>
        function grade(tis) {
            var val = $(tis).val();
            val = val.toUpperCase();
            if (val == 'A' || val == 'B' || val == 'C' || val == 'D' || val == 'A1' || val == 'B1' || val == 'C1' || val == 'D1' || val == 'A2' || val == 'B2' || val == 'C2' || val == 'D2' || val == 'E' || val == '-' || val == '') {
                $(tis).closest('tr').find('td:eq(4) input[type=text]').val(val);
                $(tis).closest('tr').find('td:eq(5) span').html(val);
            }
            else {
                $(tis).val('');
                alert('Please enter grads A, B ,C , D, A1, B1, C1, D1, A2, B2, C2, D2,E or (-) only!');
                return;
            }
        }
    </script>
