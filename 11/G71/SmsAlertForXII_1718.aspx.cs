...
public partial class common_SmsAlertFroXII_1718 : System.Web.UI.Page
{
    ...
    private void loadBranch()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                sql +=  "   where EmpCode='" + Session["LoginName"].ToString() + "' and bm.BranchCode=" + Session["BranchCode"] + "  and bm.SessionName='" + Session["SessionName"] + "' and t1.BranchCode=" + Session["BranchCode"] + " and IsClassTeacher=1 and T1.SessionName='" + Session["SessionName"] + "' and T1.Classid='" + drpclass.SelectedValue.ToString() + "'";
                BAL.objBal.FillDropDown_withValue(sql, drpBranch, "BranchName", "Id");
                drpBranch.Items.Insert(0, new ListItem("<--Select-->", "0"));
            }
        }
    ...
    {
        sql = "Select Stream,id from StreamMaster where ClassId='" + drpclass.SelectedValue.ToString() + "' and BranchCode=" + Session["BranchCode"] + " and BranchId='" + drpBranch.SelectedValue.ToString() + "' and SessionName='" + Session["SessionName"].ToString() + "'";
        BAL.objBal.FillDropDown_withValue_withSelect(sql, drpStream, "Stream", "id");
    }
    ...
    public void loadclass()
    {
        if (Session["Logintype"].ToString() == "Admin")
        {
            ...
            sql +=  " where cm.SessionName='" + Session["SessionName"] + "' and cm.BranchCode=" + Session["BranchCode"] + " and t1.BranchCode=" + Session["BranchCode"] + "  and t1.SessionName='" + Session["SessionName"] + "' and GroupId='G7' Order by CIDOrder";
        ...
        else
        {
            ...
            sql +=  " inner join dt_ClassGroupMaster t2 on t2.ClassId=t1.ClassId and t2.SessionName=t1.SessionName";
            sql +=  " where GroupId='G7' and EmpCode='" + Session["LoginName"].ToString() + "' and cm.BranchCode=" + Session["BranchCode"] + " and t2.BranchCode=" + Session["BranchCode"] + " and t1.BranchCode=" + Session["BranchCode"] + "  and cm.SessionName='" + Session["SessionName"] + "'  and t2.SessionName='" + Session["SessionName"] + "' and IsClassTeacher=1 and t1.SessionName='" + Session["SessionName"] + "' Order by CIDOrder";
            BAL.objBal.FillDropDown_withValue(sql, drpclass, "ClassName", "Id");
        }
    }
    ...
    public void loadSubjects()
    {
        ...
        //sql +=  " where sm.ClassId = '" + drpclass.SelectedValue.ToString() + "' and sm.SectionName = '" + drpsection.SelectedItem.Text.ToString() + "' and sm.SessionName = '" + Session["SessionName"].ToString() + "'";
        //sql +=  " and sgm.IsForOnlyExam = 1 and spm.IsForExam = 1 order by sgm.DisplayOrder Asc";

        sql = "Select  SubjectName,sm.Id SubjectId,'' marks from SubjectMaster sm   ";
        sql +=  "   inner join S02_SubjectPaperMaster spm on spm.S02ID=sm.PaperID and spm.SessionName=sm.SessionName   ";
        sql +=  "   inner join SubjectGroupMaster sgm on sgm.Id=spm.SubjectGroupID and sgm.SessionName=sm.SessionName  ";
        sql +=  "   where sm.Classid='" + drpclass.SelectedValue.ToString() + "' and sm.branchid='" + drpBranch.SelectedValue.ToString() + "'";
        sql +=  "   and sm.SectionName='" + drpsection.SelectedItem.Text + "' and IsForExam=1 and IsForOnlyExam=1";
        sql +=  "      and sgm.SessionName='" + Session["SessionName"] + "' and spm.BranchCode=" + Session["BranchCode"] + " and sgm.BranchCode=" + Session["BranchCode"] + " and sm.BranchCode=" + Session["BranchCode"] + " and spm.SessionName='" + Session["SessionName"] + "'  and sm.SessionName='" + Session["SessionName"] + "' order by sgm.DisplayOrder Asc,sm.id Asc";
    ...
    public void loadStudents()
    {
        ...
        sql = "select (SG.FirstName+' '+SG.MiddleName+' '+SG.LastName) as StudentName,sg.srno  as srno,'' marks, FamilyContactNo  contactno from StudentGenaralDetail SG ";
        sql +=  "   left join StudentFamilyDetails SF on SG.srno=SF.srno";
        sql +=  "   left join StudentOfficialDetails SO on SG.srno=SO.srno";
        sql +=  "   left join ClassMaster CM on SO.AdmissionForClassId=CM.Id";
        sql +=  "   left join SectionMaster SC on SO.SectionId=SC.Id";
        ...
        sql +=  "   where sc.SectionName='" + drpsection.SelectedItem.Text + "' and sg.SessionName='" + Session["SessionName"].ToString() + "' and ";
        sql +=  "   so.SessionName='" + Session["SessionName"].ToString() + "' and sf.SessionName='" + Session["SessionName"].ToString() + "' and cm.SessionName='" + Session["SessionName"].ToString() + "'";
        sql +=  "   and SC.SessionName='" + Session["SessionName"].ToString() + "' and bm.BranchCode=" + Session["BranchCode"] + " and cm.BranchCode=" + Session["BranchCode"] + " and sc.BranchCode=" + Session["BranchCode"] + " and so.BranchCode=" + Session["BranchCode"] + " and sf.BranchCode=" + Session["BranchCode"] + " and sg.BranchCode=" + Session["BranchCode"] + "  and";
        sql +=  "   sg.BranchCode=" + Session["BranchCode"].ToString() + " and so.Branch='" + drpBranch.SelectedValue.ToString() + "'";
        sql +=  "   and SO.Withdrwal is null and  isnull(SO.Promotion,'')<>'Cancelled' and SO.AdmissionForClassId='" + drpclass.SelectedValue.ToString() + "'";

        if (drpStream.SelectedIndex != 0)
        {
            sql +=  " and streamid='" + drpStream.SelectedValue.ToString() + "'";
        }

        sql +=  "   order by sg.FirstName";
        ...
        for (int i = 0; i < rptStudents.Items.Count; i++)
        {
            ...
            //sql +=  " where sm.ClassId = '" + drpclass.SelectedValue.ToString() + "' and sm.Branchid = '" + drpBranch.SelectedValue.ToString() + "' and sm.SectionName = '" + drpsection.SelectedItem.Text.ToString() + "' and sm.SessionName = '" + Session["SessionName"].ToString() + "'";
            //sql +=  " and sgm.IsForOnlyExam = 1 and spm.IsForExam = 1 order by sgm.DisplayOrder Asc";
            ...
            sql +=  "   inner join SubjectGroupMaster sgm on sgm.Id=spm.SubjectGroupID and sgm.SessionName=sm.SessionName  ";
            sql +=  "   where sm.Classid='" + drpclass.SelectedValue.ToString() + "' and sm.branchid='" + drpBranch.SelectedValue.ToString() + "'";
            sql +=  "   and sm.SectionName='" + drpsection.SelectedItem.Text + "' and sgm.BranchCode=" + Session["BranchCode"] + " and spm.BranchCode=" + Session["BranchCode"] + " and sm.BranchCode=" + Session["BranchCode"] + " and IsForExam=1 and IsForOnlyExam=1";
            sql +=  "   order by sgm.DisplayOrder Asc,sm.id Asc";
    ...
    public string GetMMarks(string Eval, string subjectid)
    {
        ...
        ds = DLL.objDll.Sp_SelectRecord_usingExecuteDataset("USP_CCEXIIMMMarksForMessage", param);
    ...
    public string GetTestMarks(string Eval, string subjectid,string srno)
    {
        ...
        ds = DLL.objDll.Sp_SelectRecord_usingExecuteDataset("USP_CCEXIIMarksForMessage", param);
