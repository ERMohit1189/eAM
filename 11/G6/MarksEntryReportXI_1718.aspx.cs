...
public partial class common_MarksEntryReportXI_1718 : Page
{
    ...
    public void loadclass()
    {
        if (Session["Logintype"].ToString() == "Admin")
        {
            ...
            sql +=  " where cm.SessionName='" + Session["SessionName"] + "' and t1.BranchCode=" + Session["BranchCode"] + " and cm.BranchCode=" + Session["BranchCode"] + " and t1.SessionName='" + Session["SessionName"] + "' and GroupId='G6' Order by CIDOrder";
    ...
    {
        sql = "Select MaxMarks1,MaxMarks2,MaxMarks3,MaxMarks4 from SetMaxMinMarks_XI where Eval='" + Eval + "' and BranchCode=" + Session["BranchCode"] + " and paperid='" + drpSubject.SelectedValue.ToString().Trim() + "' and SessionName='" + Session["SessionName"].ToString() + "'";
    ...
    protected void drpSubject_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (drpEval.SelectedIndex != 0)
            {
                ...
                loadMarks(GridView1, drpEval.SelectedValue.ToString(), "lblT1", "lblT2", "lblHY", "lblPrac", "Label2", "Label3", "lblTotal", "lblGrade", "lblMMT1", "lblMMT2", "lblMMHY", "lblMMPrac", "lblConinBoardMarks", "lblGTotal1");
            }
            else
            {
                GridView1.Visible = false;
                GridView2.Visible = true;
                loadMarks(GridView2, "TERM1", "lblT1", "lblT2", "lblHY", "lblPracMarks", "Label2", "Label3", "lblTotal", "lblGrade", "lblMMT1", "lblMMT2", "lblMMHY", "lblPracMM", "lblConinBoardMarks", "lblGTotal1");
                loadMarks(GridView2, "TERM2", "lblT4", "lblT5", "lblHY2", "lblPracMarks2", "lblBestofTwo", "lblConinten", "lblTotal2", "lblGrade2", "lblMMT4", "lblMMT5", "lblMMHY2", "lblPracMM2", "lblConinBoardMarks2", "lblGTotal2");
            }
        }
        
    }
    protected void loadMarks(GridView grd,string Eval,string test1,string test2,string hy, string prac,string total,string tenper, string finaltotal, string grademarks, string mmt1, string mmt2, string mmhy,string mmprac, string coninboard, string GTotal1)
    {
        if (grd.Rows.Count > 0)
        {
            ...
            foreach (GridViewRow gvr in grd.Rows)
            {
                ...
                sql = "Select Test1,Test2,SAT,Prac from CCEXI_1718 where SRNO='" + lblsrno.Text + "' and Evaluation='" + Eval + "' and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "' and paperid='" + drpSubject.SelectedValue.ToString() + "'";
            ...
            refresh(grd, Eval, test1, test2,hy, prac, mmt1, mmt2, mmhy, mmprac, total, tenper, finaltotal, grademarks, coninboard, GTotal1);
            createTitle();
        }

    }
    private void refresh(GridView grd, string Eval, string test1, string test2, string hy, string prac, string mmt1, string mmt2, string mmhy, string mmprac, string total, string tenper, string finaltotal, string grademarks,string coninboard, string GTotal1)
    {
        if (grd.Rows.Count > 0)
        {
            Label lblMMT1 = (Label)grd.HeaderRow.FindControl(mmt1);
            ...
            for (int i = 0; i < grd.Rows.Count; i++)
            {
                ...
                double num1 = 0, num2 = 0,  num6 = 0, num7 = 0;
                bool result;
                ...
                {
                    if ((Label3.Text.ToUpper() == "NP" || Label3.Text == ""))
                    {
    ...
    protected void GridView1_RowCreated(object sender, GridViewRowEventArgs e)
    {
       ...
       // double avgMarks = double.Parse((_oo.ReturnTag(avg, "avgMarks").ToString() == "" ? "0" : _oo.ReturnTag(avg, "avgMarks")));
        ...
        avg1 = avg1 + " select sum((((TRY_CONVERT(decimal(10,0),(case when isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)>isnull(TRY_CONVERT(decimal(10,0),m.Test2),0)  ";
        avg1 = avg1 + " then(isnull(TRY_CONVERT(decimal(10, 0), m.Test1), 0) * 10) / isnull(TRY_CONVERT(decimal(10, 0), mm.MaxMarks1), 1)  else  (isnull(TRY_CONVERT(decimal(10, 0), m.Test2), 0) * 10) / isnull(TRY_CONVERT(decimal(10, 0), mm.MaxMarks2), 1) end)) ";
        avg1 = avg1 + " +isnull(TRY_CONVERT(decimal(10, 0), m.SAT), 0))  / (10 +isnull(TRY_CONVERT(decimal(10, 0), MaxMarks3), 1))) ";
        avg1 = avg1 + " *isnull(TRY_CONVERT(decimal(10, 0), MaxMarks3), 1)) +isnull(TRY_CONVERT(decimal(10, 0), m.Prac), 0)) avgMarks ";
        avg1 = avg1 + " from CCEXI_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode inner join SetMaxMinMarks_XI mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
        avg1 = avg1 + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.Evaluation='TERM1' and m.SubjectId=" + drpSubjectGroup.SelectedValue.ToString() + "  ";
        avg1 = avg1 + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "'  ";
        avg1 = avg1 + " and SrNo in (select SrNo from AllStudentRecord_UDF('" + Session["SessionName"] + "'," + Session["BranchCode"] + ") stu ";
        avg1 = avg1 + " where stu.sessionName='" + Session["SessionName"] + "' and isnull(stu.Promotion, '')<>'Cancelled' ";
        avg1 = avg1 + " and stu.ClassId=" + drpclass.SelectedValue + " and stu.SectionName='" + drpsection.SelectedItem.Text + "'  and BranchId=" + drpBranch.SelectedValue + " ";
        avg1 = avg1 + " and isnull(Withdrwal,'') = '' ";
        avg1 = avg1 + " and isnull(blocked,'') = '') ";
        avg1 = avg1 + ")T1 ";
    ...
    {
        oo.ExporttolandscapePdf(Response, "SubjectwiseCumlativeXI", table1);
    }
    protected void ImageButton1_Click(object sender, EventArgs e)
    {
        oo.ExportTolandscapeWord(Response, "SubjectwiseCumlativeXI", table1);
    }
    protected void ImageButton2_Click(object sender, EventArgs e)
    {
        GridView1.Style.Add("text-transform", "uppercase");
        oo.ExportDivToExcelWithFormatting(Response, "SubjectwiseCumlativeXI.xls", table1, Server.MapPath("~/Admin/css/style.css"));
    }
