...
public partial class common_MarksEntryXI_1718 : Page
{
    ...
    public void loadclass()
    {
        if (Session["Logintype"].ToString() == "Admin")
        {
            ...
            sql +=  " where cm.SessionName='" + Session["SessionName"] + "' and cm.BranchCode=" + Session["BranchCode"] + " and GroupId='G6' Order by CIDOrder";
        ...
        else
        {
            ...
            sql +=  " where GroupId='G6'  and sctm.SessionName='" + Session["SessionName"].ToString() + "' and sctm.BranchCode=" + Session["BranchCode"] + " and Ecode='" + Session["LoginName"].ToString() + "' Order by CIDOrder";
            oo.FillDropDown_withValue(sql, drpclass, "ClassName", "Id");
        }
    }
    ...
    //        {
    //            string sql33 = "select dr.Id, ClassName, dr.EvalName, CountStartFromAdmission, format(ExamStartDate, 'dd-MMM-yyyy')ExamStartDate, format(ExamEndDate, 'dd-MMM-yyyy')ExamEndDate, format(ExamLockDate, 'dd-MMM-yyyy')ExamLockDate, CountStartFrom, format(CountStartDateDate, 'dd-MMM-yyyy')CountStartDateDate, format(dr.RecordDate, 'dd-MMM-yyyy hh:mm:ss tt')RecordDate, dr.LoginName from SetAttendenceRange dr inner join ClassMaster cm on cm.id=dr.Classid and cm.SessionName=dr.SessionName and cm.BranchCode=dr.BranchCode Where EvalName='" + drpEval.SelectedValue.ToString().Trim() + "' and Classid=" + drpclass.SelectedValue.ToString() + " and dr.SessionName='" + Session["SessionName"].ToString() + "' and dr.BranchCode=" + Session["BranchCode"].ToString() + "";
    ...
    //        if (IsLocked)
    //        {
    //        if (drpEval.SelectedValue.ToString() == "TERM1")
    //        {
    //            if (GridView1.Rows.Count > 0)
    //            {
    //                foreach (GridViewRow gvr in GridView1.Rows)
    //                {
    //                    TextBox txtUt1Max = (TextBox)gvr.FindControl("txtUt1Max");
    //                    TextBox txtUt1 = (TextBox)gvr.FindControl("txtUt1");
    //                    TextBox txtUt2Max = (TextBox)gvr.FindControl("txtUt2Max");
    //                    TextBox txtUt2 = (TextBox)gvr.FindControl("txtUt2");
    //                    TextBox txtPracMax = (TextBox)gvr.FindControl("txtPracMax");
    //                    TextBox txtPrac = (TextBox)gvr.FindControl("txtPrac");
    //                    TextBox txtHYMax = (TextBox)gvr.FindControl("txtHYMax");
    //                    TextBox txtHY = (TextBox)gvr.FindControl("txtHY");
    //                    //txtUt1Max.Enabled = false;
    //                    //txtUt1.Enabled = false;
    //                    //txtUt2Max.Enabled = false;
    //                    //txtUt2.Enabled = false;
    //                    //txtPracMax.Enabled = false;
    //                    //txtPrac.Enabled = false;
    //                    //txtHYMax.Enabled = false;
    //                    //txtHY.Enabled = false;

    //                }
    //            }
    //        }
    //        if (drpEval.SelectedValue.ToString() == "TERM2")
    //        {
    //            if (GridView1.Rows.Count > 0)
    //            {
    //                foreach (GridViewRow gvr in GridView1.Rows)
    ...
    private void CheckExamLockStatus()
    {
        if (Session["Logintype"] != null && Session["Logintype"].ToString() == "Staff")
        {
            ...
            if (!string.IsNullOrEmpty(ExamLockDate))
            {
                ...
                // bool isValidDate = DateTime.TryParseExact(ExamLockDate, "dd-MMM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out specificDate);
               // string[] formats = { "dd-MMM-yyyy", "yyyy-MM-dd", "MM/dd/yyyy", "dd/MM/yyyy" }; // Add possible formats
               // bool isValidDate = DateTime.TryParseExact(ExamLockDate, formats, CultureInfo.InvariantCulture, DateTimeStyles.None, out specificDate);
                if (isValidDate)
                {
                    if (currentDate >= specificDate)
                    {
                ...
                //DateTime currentDate = DateTime.Now.Date;
                //DateTime specificDate;  // Declare variable before usage (C# 4.0 requirement)

                //if (DateTime.TryParseExact(ExamLockDate, "dd-MMM-yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out specificDate))
                //{
                //    if (currentDate >= specificDate)
                //    {
                //        IsLocked = true;
