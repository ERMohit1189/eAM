...
public partial class ClassWiseStudentsMarksList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                Response.Redirect("Studentwisecumulative_VItoVIII.aspx?check=ClassWiseStudentsMarksList_VItoVIII");
            }
    ...
    public void loadclass()
    {
        ...
        else
        {
            if (Session["SessionName"].ToString() == "2015-2016")
            {
                ...
                sql +=  " where cm.SessionName='" + Session["SessionName"] + "' and sctm.SessionName='" + Session["SessionName"].ToString() + "' and Ecode='" + Session["LoginName"].ToString() + "' and ClassTeacher='Yes' Order by CIDOrder";
                oo.FillDropDown_withValue(sql, drpClass, "ClassName", "Id");
            }
            else
            {
                ...
                sql +=  " where EmpCode='" + Session["LoginName"].ToString() + "' and ctm.SessionName='" + Session["SessionName"].ToString() + "' and ctm.BranchCode=" + Session["BranchCode"] + " and IsClassTeacher=1";
                oo.FillDropDown_withValue(sql, drpClass, "ClassName", "Id");
            }
        }
    }
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        if (RadioButtonList1.SelectedIndex == 0)
        {
            ...
            Response.Write("window.open('ReportCardofVItoVIII.aspx','_blank')");
            Response.Write("</script>");
        }
        else if (RadioButtonList1.SelectedIndex == 1)
        {
            Response.Write("<script>");
            Response.Write("window.open('ReportCardofVItoVIIIPer.aspx','_blank')");
            Response.Write("</script>");
        }
        else
        {
            Response.Write("<script>");
            Response.Write("window.open('ParticularStudentMarks_VItoVIII.aspx','_blank')");
            Response.Write("</script>");
        }
