<%@ Page Title="" Language="C#" MasterPageFile="~/Master/admin_root-manager.master" AutoEventWireup="true" CodeFile="Studentwisecumulative_XI.aspx.cs" Inherits="common_G6_Studentwisecumulative_XI" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderMainBox" runat="Server">
    ...
    <script>
        ...
        function LoadSRno() {
            $.ajax({
                    type: "POST",
                    url: '<%=ResolveUrl("Server/ddlSrNo_6.aspx") %>',
                data: { 'classId': $("#drpclass").val(), 'SectionId': $("#drpsection").val(), 'Status': $("#drpStatus").val(), 'BranchId': $("#drpBranch").val(), 'StreamId': "" },
                    dataType: "json",
                    async: true,
                    success: function (result) {
                    },
        ...
        function LoadClass() {
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/ddlClass_6.aspx") %>',
                data: {},
                dataType: "json",
                async: true,
                success: function (result) {
        ...
        function LoadSection() {
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/ddlSection_6.aspx") %>',
        ...
        function LoadBranch() {
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/ddlBranch_6.aspx") %>',
                data: { 'classId': $("#drpclass").val() },
                dataType: "json",
                async: true,
                success: function (result) {
        ...
        function PrintDiv() {
            ...
            printWindow.document.write('<html><head><title>STUDENT WISE CUMLATIVE FOR XI</title>' + headContent + '</head>');
        ...
        function LoadData() {
            ...
            var AttendanceTyp = $("#drpAttendance").val();
            var BranchID = $("#drpBranch").val();
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/Studentwisecumulative_XI_1718Server.aspx") %>',
                data: {
                    'ClassId': ClassId,
                    'ClassName': ClassName,
                    'SectionID': SectionID,
                ...
                success: function (result) {
                    HideLoader();
                    $("#divExport").html(result.responseText);
                },
                error: function (result) {
                    HideLoader();
                    $("#divExport").html(result.responseText);
