    ...
    protected void LinkButton5_Click(object sender, EventArgs e)
    {
        ...
        if (rptType.SelectedValue == "Class wise Attendance")
        {
            ...
            else
            {
                ...
                sql += "  left join AttendanceDetailsDateWise ad on SG.SrNo=ad.SrNo  and SG.BranchCode=ad.BranchCode and ad.AttendanceDate between '" + fromdate + "'   and   '" + fromdate + "' and ad.CategoryWise='Date Wise'    ";
                sql += "  where sg.ClassName='" + drpClass.SelectedItem.ToString() + "' and  SG.SectionName='" + drpSection.SelectedItem.ToString() + "' and (SG.BranchId='" + drpStream.SelectedValue.ToString() + "' or SG.BranchId is null) ";
                sql += "  and (SG.Promotion is null or SG.Promotion<>'Cancelled') and SG.Withdrwal is null ";
                ...
                {
                    Label14.Text = "Student's Attendance Report | " + DateTime.Parse(fromdate).ToString("dd MMM yyyy") + "| Class: " + drpClass.SelectedItem.Text.ToString() + " " + (drpStream.SelectedItem.Text.ToString().ToUpper() == "OTHER" ? "" : (drpStream.SelectedItem.Text.ToString().ToUpper() == "NA") ? "" : " " + drpStream.SelectedItem.Text.ToString()) + " (" + drpSection.SelectedItem.Text.ToString() + ")";
        ...
        {
            if (drpBatch.SelectedItem.ToString() == "<--Select>")
            {
                Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Please Select Condition", "A");
            ...
            else
            {
                ...
                sql += "  convert(nvarchar,ad.AttendanceDate,106) as AttendanceDate from AllStudentRecord_UDF(''," + Session["BranchCode"].ToString() + ") SG";
                sql += "  left join AttendanceDetailsDateWise ad on SG.SrNo=ad.SrNo  and SG.BranchCode=ad.BranchCode and ad.AttendanceDate between '" + fromdate + "'   and   '" + fromdate + "' and ad.CategoryWise='Date Wise'";
                sql += "  where  BatchName='" + drpBatch.SelectedItem.ToString() + "' and (SG.Promotion is null or SG.Promotion<>'Cancelled') and SG.Withdrwal is null";
                sql += "  Order by SG.Name Asc,AttendanceValue desc";
