...
public partial class admin_GenerateTimeTable : Page
{
    string sql = "", _sql = "", Sql = "";
    ...
    protected void btnGenerate_Click(object sender, EventArgs e)
    {
        ...
        {
            cmd.CommandText = "TTGenerateTimeTableProc";
            ...
            try
            {
                ...
                cmd.ExecuteNonQuery();
                _con.Close();
                Campus camp = new Campus(); camp.msgbox(Page, msgbox, "Generated successfully.", "S");
                LoadData();
            }
            catch (Exception ex)
            {
    ...
    {
        GridViewTT.DataSource = null;
        GridViewTT.DataBind();
        GridViewTT.Visible = true;

        GridViewTT.Columns.Clear();
        GridViewTT.DataBind();

        BoundField sr = new BoundField();
        sr.DataField = "sr";
        sr.HeaderText = "Class";
        GridViewTT.Columns.Add(sr);
        GridViewTT.DataBind();
        ...
        for (int k = 0; k < DTperiod.Rows.Count; k++)
        {
            ...
            tests.DataField = DTperiod.Rows[k]["Period"].ToString();
            tests.HeaderText = DTperiod.Rows[k]["Period"].ToString();
            GridViewTT.Columns.Add(tests);
        ...
        for (int i = 0; i < DTClass.Rows.Count; i++)
        {
            ...
            for (int j = 0; j < DTBranchn.Rows.Count; j++)
            {
                ...
                for (int k = 0; k < DTSection.Rows.Count; k++)
                {
                    ...
                    {
                        sql = "select PaperCode+' | '+EmpCode data1, StartFrom+'-To-'+EndTo data2  from TTGenerateTimeTable where PeriodName='" + table.Columns[l] + "' ";
                        sql = sql + " and convert(date, RecordedDate)=convert(date, GETDATE()) and classid=" + DTClass.Rows[i]["id"].ToString() + " and BranchId=" + DTBranchn.Rows[j]["id"].ToString() + " ";
                        sql = sql + " and sectionId=" + DTSection.Rows[k]["id"].ToString() + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "' order by ClassId, BranchId, Sectionid asc";
                        var DTdata = _oo.Fetchdata(sql);
                        if (DTdata.Rows.Count > 0)
                        {
                            sb.AppendLine("<td>" + DTdata.Rows[0]["data1"].ToString() + "<br>" + DTdata.Rows[0]["data2"].ToString()+"</td>");
                        }
                        else
                        {
                            sb.AppendLine("<td></td>");
