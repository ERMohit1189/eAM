...
public partial class AutomaticTimeTable : Page
{
    string sql = "", _sql = "", Sql = "";
    ...
    protected void loadStaff()
    {
        ...
        sql = sql + " (select EmpCode from TTGenerateTimeTable where SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + " ";
        sql = sql + " and convert(date, RecordedDate)=convert(date, '" + (txtDate.Text.Trim()==""?curDate: txtDate.Text.Trim()) + "')) ";
            ...
            for (int j = 0; j < DTBranchn.Rows.Count; j++)
            {
                ...
                for (int k = 0; k < DTSection.Rows.Count; k++)
                {
                    ...
                    for (int l = 1; l < table.Columns.Count; l++)
                    {
                        ...
                        sql = sql + " and convert(date, RecordedDate)=convert(date, '"+ txtDate.Text.Trim() + "') and classid=" + DTClass.Rows[i]["id"].ToString() + " and BranchId=" + DTBranchn.Rows[j]["id"].ToString() + " ";
                        sql = sql + " and sectionId=" + DTSection.Rows[k]["id"].ToString() + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "' ";
                        if (ddlStaff.SelectedIndex!=0)
                        {
                            sql = sql + " and empCode='" + ddlStaff.SelectedValue + "' ";
                        }
                        sql = sql + "  order by ClassId, BranchId, Sectionid asc";
