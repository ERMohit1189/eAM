...
namespace _1
{
    public partial class AdminMonthwiseStudentAttendenceReportNew : Page
    {
        ...
        {
            _sql = "Select Distinct ClassName,cm.Id as Id,CIDOrder from ClassTeacherMaster ctm";
            _sql = _sql + " inner join ClassMaster cm on cm.Id=ctm.ClassId and cm.SessionName=ctm.SessionName";
            _sql = _sql + " where ctm.SessionName='" + Session["SessionName"].ToString() + "' and EmpCode='" + Session["LoginName"].ToString() + "' and ctm.BranchCode=" + Session["BranchCode"].ToString() + "";
            _sql = _sql + " and IsClassTeacher=1 Order by CIDOrder";
        ...
        {
            _sql = "Select BranchName,bm.Id as Id from ClassTeacherMaster ctm";
            _sql = _sql + " inner join BranchMaster bm on bm.Id=ctm.BranchId and bm.SessionName=ctm.SessionName";
            _sql = _sql + " where ctm.SessionName='" + Session["SessionName"].ToString() + "' and EmpCode='" + Session["LoginName"].ToString() + "' and ctm.BranchCode=" + Session["BranchCode"].ToString() + " and IsClassTeacher=1";
        ...
        {
            _sql = "Select SectionName,bm.Id as Id from ClassTeacherMaster ctm";
            _sql = _sql + " inner join SectionMaster bm on bm.ClassNameId=ctm.ClassId and bm.id=ctm.SectionId and bm.SessionName=ctm.SessionName and bm.BranchCode=ctm.BranchCode";
            _sql = _sql + " where ctm.SessionName='" + Session["SessionName"].ToString() + "' and EmpCode='" + Session["LoginName"].ToString() + "' ";
            _sql = _sql + " and IsClassTeacher=1 and ctm.ClassId="+ drpClass.SelectedValue + " and ctm.BranchCode=" + Session["BranchCode"].ToString() + "";
        ...
        public void Loadgrid()
        {
            ...
            if (RadioButtonList1.SelectedIndex == 0)
            {
                ...
                _sql = _sql + "   where ltrim(rtrim(cm.ClassName))='" + drpClass.SelectedItem.Text.Trim() + "'  and sc.SectionName='" + drpSection.SelectedItem + "'  and (So.Branch='" + drpStream.SelectedValue + "' or So.Branch is null) and";
                ...
                _sql = _sql + "   so.BranchCode=" + Session["BranchCode"] + " and sg.BranchCode=" + Session["BranchCode"] + " and cm.BranchCode=" + Session["BranchCode"] + " and sc.BranchCode=" + Session["BranchCode"] + " ";
                _sql = _sql + "   and SO.Withdrwal is null and (SO.Promotion is null or SO.Promotion<>'Cancelled')";
                _sql = _sql + "   and convert(date, FORMAT(SO.DateOfAdmiission, 'yyyy-MM-dd'))< convert(date, DATEADD(s, -1, DATEADD(mm, DATEDIFF(m, 0, '28-"+ month + "-"+ year + "') + 1, 0))) order by FirstName";
            }
            else
            {
                ...
                _sql = _sql + "   where so.srno='" + lblsrno.Text + "' and ltrim(rtrim(cm.ClassName))='" + lblclass.Text.Trim() + "'  and sc.SectionName='" + lblsection.Text + "' and (bm.BranchName='" + lblBranch.Text + "' or bm.BranchName is null)";
        ...
        public void Name(string branchid)
        {
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                //string sqls = "select count(distinct convert(date, AttendanceDate)) as workingdays from AttendanceDetailsDateWise where AttendanceMonth = '"+ month + "' and SessionName = '"+Session["SessionName"]+ "'  and BranchCode = " + Session["BranchCode"] + " and ltrim(rtrim(ClassName)) = '" + classname + "'";
                Label lblTotalWorkingDays = (Label)GridView1.Rows[i].FindControl("lblTotalWorkingDays");
                //lblTotalWorkingDays.Text = _oo.ReturnTag(sqls, "workingdays").ToString();
                
                //_sql = "select  format(DateOfAdmiission, 'MMM') mon,format(DateOfAdmiission, 'yyyy') yer from StudentOfficialDetails where SrNo='" + lblsrno.Text.Trim() + "' and SessionName='" + Session["SessionName"] + "' and BranchCode = " + Session["BranchCode"] + " and TypeOFAdmision='New'";
                //if (_oo.ReturnTag(_sql, "mon").ToString()==month && _oo.ReturnTag(_sql, "yer").ToString()==years)
                //{
                //    _sql = "SELECT DATEDIFF(DD, '01-'+format(DateOfAdmiission, 'MMM')+'-'+format(DateOfAdmiission, 'yyyy'), format(DateOfAdmiission, 'dd-MMM-yyyy')) AS DateDiff from StudentOfficialDetails where SrNo='" + lblsrno.Text.Trim() + "' and SessionName='" + Session["SessionName"] + "' and BranchCode = " + Session["BranchCode"] + " and TypeOFAdmision='New'";
                //    lblTotalWorkingDays.Text = (int.Parse(lblTotalWorkingDays.Text) - (int.Parse(_oo.ReturnTag(_sql, "DateDiff"))+1)).ToString();
                //}
                lblTotalWorkingDays.Text = W.ToString();
                ...
                lblTotalAbsent.Text = A.ToString(); //(double.Parse(lblTotalWorkingDays.Text)-double.Parse(lblTotalPresent.Text)).ToString("0");
            }
        }
        ...
        public string Addvalue(string year, string month, string classname, string section, string branchid, string lblsrno, string attendencedate, string lblday, int i, int j)
        {
            ...
            _sql = "Select ar.AttendanceValue, convert(date, AttendanceDate) AttendanceDate from AttendanceDetailsDateWise ar left join StudentOfficialDetails SO on So.srno=ar.srno and so.sessionname=ar.sessionname";
            _sql = _sql + " where DATEPART(yyyy,ar.AttendanceDate)='" + year + "' and ltrim(rtrim(ar.AttendanceMonth))='" + month + "' and ltrim(rtrim(ar.ClassName))='" + classname + "'";
            _sql = _sql + " and ar.SrNo='" + lblsrno + "' and ar.SessionName='" + Session["SessionName"] + "'  and SO.SessionName = '" + Session["SessionName"] + "' and SO.BranchCode = " + Session["BranchCode"] + " and ar.BranchCode = " + Session["BranchCode"] + "";
            _sql = _sql + " and DATEPART(dd,ar.AttendanceDate)='" + attendencedate + "'";

            string sql2 = "";
            sql2 = "select * from AttendanceDetailsDateWise where SessionName = '" + Session["SessionName"] + "' and SrNo = '" + lblsrno + "' and convert(date, AttendanceDate)='" + attendencedate + "' and BranchCode = " + Session["BranchCode"] + "";
            string sql3 = "";
            sql3 = "select * from AttendanceDetailsDateWise where SessionName = '" + Session["SessionName"] + "' and convert(date, AttendanceDate)='" + attendencedate + "' and BranchCode = " + Session["BranchCode"] + "";
            ...
            if (attendencedate!= "&nbsp;")
            {
                ...
                if (int.Parse(attendencedate) < 10)
                {
                    attendencedate = ("0" + attendencedate);
                ...
                if (int.Parse(monthf) < 10)
                {
                 ...
                 string AttendanceDate = (yearf + "-"+ monthf+"-"+ attendencedate).ToString();
                ...
                if (_oo.ReturnTag(_sql2, "day").ToString() == attendencedate && _oo.ReturnTag(_sql2, "mon").ToString() == month && _oo.ReturnTag(_sql2, "yer").ToString() == year)
                {
                    lbl.Text = "NAD";
                }
