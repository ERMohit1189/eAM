...
public partial class SuperAdmin_menu_accessability : Page
{
    ...
    public void CheckBoxCheck()
    {
        if (TreeView1.Nodes.Count > 0)
        {
            ...
            for (int i = 0; i < TreeView1.Nodes.Count; i++)
            {
                sql = "Select Count(*) as Counter_Value  From menu_permission where Login_Id='" + DropDownList1.SelectedItem.Value + "' and Permission_Value='Yes' and Menu_Id='" + TreeView1.Nodes[i].Value + "' and BranchCode=" + ddlBranch.SelectedValue + "";
                ...
                for (int j = 0; j < TreeView1.Nodes[i].ChildNodes.Count; j++)
                {
                    sql = "Select Count(*) as Counter_Value  From menu_permission where Login_Id='" + DropDownList1.SelectedItem.Value + "' and Permission_Value='Yes' and Menu_Id='" + TreeView1.Nodes[i].ChildNodes[j].Value + "' and BranchCode=" + ddlBranch.SelectedValue + "";
                    if (oo.ReturnTag(sql, "Counter_Value") == "1")
                    {
                    ...
                    for (int k = 0; k < TreeView1.Nodes[i].ChildNodes[j].ChildNodes.Count; k++)
                    {
                        sql = "Select Count(*) as Counter_Value  From menu_permission where Login_Id='" + DropDownList1.SelectedItem.Value + "' and Permission_Value='Yes' and Menu_Id='" + TreeView1.Nodes[i].ChildNodes[j].ChildNodes[k].Value + "' and BranchCode=" + ddlBranch.SelectedValue + "";
                        if (oo.ReturnTag(sql, "Counter_Value") == "1")
                        {
    ...
    protected void Button1_Click(object sender, System.EventArgs e)
    {
            ...
            if (TreeView1.Nodes.Count > 0)
            {
                ...
                if (oo.ReturnTag(sql, "Counter") == "0")
                {
                    ...
                    for (int i = 0; i < TreeView1.Nodes.Count; i++)
                    {
                        ...
                        Permission_Header.Parameters.AddWithValue("@BranchCode", ddlBranch.SelectedValue);
                        ...
                        for (int j = 0; j < TreeView1.Nodes[i].ChildNodes.Count; j++)
                        {
                            ...
                            Permission_Sub.Parameters.AddWithValue("@BranchCode", ddlBranch.SelectedValue);
                            ...
                            for (int k = 0; k < TreeView1.Nodes[i].ChildNodes[j].ChildNodes.Count; k++)
                            {
                                ...
                                Permission_Store.Parameters.AddWithValue("@BranchCode", ddlBranch.SelectedValue);
                    ...
                    Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Menu submitted Successfully.", "S");
                }
                else
                {
                    for (int i = 0; i < TreeView1.Nodes.Count; i++)
                    {
                        sql = "Select Id From menu_permission where Login_Id='" + DropDownList1.SelectedValue + "' and Menu_Id='" + TreeView1.Nodes[i].Value + "' and BranchCode=" + ddlBranch.SelectedValue + "";
                        ...
                        for (int j = 0; j < TreeView1.Nodes[i].ChildNodes.Count; j++)
                        {
                            sql = "Select Id From menu_permission where Login_Id='" + DropDownList1.SelectedValue + "' and Menu_Id='" + TreeView1.Nodes[i].ChildNodes[j].Value + "' and BranchCode=" + ddlBranch.SelectedValue + "";
                            if (oo.ReturnTag(sql, "Id") != "")
                            {
                                SqlCommand Permission_Sub = new SqlCommand("menu_permission_proc", con);
                                Permission_Sub.CommandType = CommandType.StoredProcedure;
                            ...
                            for (int k = 0; k < TreeView1.Nodes[i].ChildNodes[j].ChildNodes.Count; k++)
                            {
                                sql = "Select Id From menu_permission where Login_Id='" + DropDownList1.SelectedValue + "' and BranchCode=" + ddlBranch.SelectedValue + " and Menu_Id='" + TreeView1.Nodes[i].ChildNodes[j].ChildNodes[k].Value + "'";
                                if (oo.ReturnTag(sql, "Id") != "")
                                {
                                    SqlCommand Permission_Store = new SqlCommand("menu_permission_proc", con);
                                    Permission_Store.CommandType = CommandType.StoredProcedure;
                    ...
                    Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Menu updated successfully.", "S");
                }
            }
        }
        else
        {
            ...
            Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Please Select  User Name!.", "A");
        }
    }
    ...
    private void loadUserName()
    {
        ...
        if (DropDownList2.SelectedIndex == 1)
        {
            ...
            sql = sql + " Inner join EmpGeneralDetail EG on EG.EmpId=EO.EmpId and EG.BranchCode=EO.BranchCode";
            sql = sql + " Where EO.Ecode=(select LoginName from LoginTab where LoginId='" + DropDownList1.SelectedValue.ToString() + "')";
            sql = sql + " and EO.BranchCode=" + ddlBranch.SelectedValue + " and EO.Password=(select Pass from LoginTab where LoginId='" + DropDownList1.SelectedValue.ToString() + "')";
            username = oo.ReturnTag(sql, "EmpName").Trim();
        }
        else
        {
            sql = "Select Name from NewAdminInformation";
            sql = sql + " Where UserId=(select LoginName from LoginTab where LoginId='" + DropDownList1.SelectedValue.ToString() + "')";
            sql = sql + " and BranchId=" + ddlBranch.SelectedValue + " and Password=(select Pass from LoginTab where LoginId='" + DropDownList1.SelectedValue.ToString() + "')";
