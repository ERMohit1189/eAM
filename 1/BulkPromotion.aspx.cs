...
public partial class BulkPromotion : System.Web.UI.Page
{
    ...
    protected void Loadrecord()
    {
        try
        {
            ...
            {
                string sql = "select * from AllStudentRecord_UDF('" + drpSession.SelectedValue + "'," + Session["BranchCode"] + ") where ClassId=" + DrpClass.SelectedValue + " and BranchId=" + DropBranch.SelectedValue + "  and SectionId=" + drpSection.SelectedValue + " ";
                ...
                {
                    sql += " and Medium='" + DropDownList1.SelectedItem.Text + "' ";
                }
                if (DropDownList2.SelectedValue != "0")
                {
                    sql += " and Actname='" + DropDownList2.SelectedItem.Text + "' ";
                }
    ...
    {
        DropDownList2.Items.Clear(); DropDownList3.Items.Clear(); 
        _sql = "select * from tblEducationAct where BranchCode= " + Session["BranchCode"] + "";

        _oo.FillDropDown_withValue(_sql, DropDownList2, "Actname", "id");
        _oo.FillDropDown_withValue(_sql, DropDownList3, "Actname", "id");
    ...
    private void LoadClass()
    {
        ...
        _sql = "Select ClassName,Id from ClassMaster Where SessionName='" + drpSession.SelectedItem.Text + "' and BranchCode=" + Session["BranchCode"] + " order by CIDOrder asc ";
    ...
    private void LoadDefaultMedium()
    {
        ...
        BAL.objBal.FillDropDown_withValue(_sql, DropDownList1, "Medium", "id");
        DropDownList1.Items.Insert(0, new ListItem("<--Select-->", ""));
        using (var objBll = new BLL())
        {
            try
            {
                objBll.loadDefaultvalue("Medium", DropDownList1);
            }
            catch
            {
                // ignored
    ...
    private void LoadNewSessionClass()
    {
        ...
        _sql +=  " where (Course='" + DropCourseNew.SelectedValue + "' or Course is NULL) " +
            "and SessionName='" + drpSessionNew.SelectedItem.Text + "' " +
            "and BranchCode=" + Session["BranchCode"] + " and CIDOrder !=0 ";
    ...
    {
        _sql = "select * from MediumMaster where BranchCode=" + Session["BranchCode"] + "";
        ...
        //_sql = "select top 1 * from HouseMaster where  BranchCode=" + Session["BranchCode"] + " order by Id desc";
    ...
    protected void LinkSubmit_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            using (SqlCommand cmd = new SqlCommand())
            {
                for (int i = 0; i < mainGrid.Items.Count; i++)
                {
                    ...
                    if (drpstatus.SelectedValue == "Pass")
                    {
                        ...
                        for (int j = 0; j < childGrid.Items.Count; j++)
                        {
                            ...
                            cmd.Parameters.AddWithValue("@OldSessionName", drpSession.SelectedValue);
                          ...
                          //  cmd.Parameters.AddWithValue("@NewHouseName", DropHouseNew.SelectedItem.Text);
                            ...
                            {
                                cmd.Parameters.AddWithValue("@EducationActId", DropDownList3.SelectedValue);
                            }
                            try
                            {
                                _con.Open();
                ...
                Loadrecord();
                Campus camp = new Campus(); camp.msgbox(Page, msgbox1, "Promoted Successfully.", "S");
            }
        }
    }
