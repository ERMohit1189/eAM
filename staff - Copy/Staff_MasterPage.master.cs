...
public partial class Staff_MasterPage : System.Web.UI.MasterPage
{
    ...
    private void ADDTopMenuItem()
    {
        ...
        if (Repeater1.Items.Count > 0)
        {
            for (int i = 0; i < Repeater1.Items.Count; i++)
            {
                ...
                AddfirstChildMenu(Repeater2, lblId.Text.Trim(), table, isfor, value);
            }

        }
    }

    private void AddfirstChildMenu(Repeater Repeater2, string parentid, string table, string isfor, string value)
    {
        ...
        if (Repeater2.Items.Count > 0)
        {
            ...
            {
                Repeater Repeater3 = (Repeater)Repeater2.Items[i].FindControl("Repeater3_ss");
                Label lblId = (Label)Repeater2.Items[i].FindControl("Label2_2");
                AddSecondChildNode(Repeater3, lblId.Text.Trim(), table, isfor, value);
                if (Repeater3.Items.Count == 0)
                {
                    HtmlGenericControl sub_child_menu_badge = (HtmlGenericControl)Repeater2.Items[i].FindControl("sub_child_menu_badge");
                    sub_child_menu_badge.Visible = false;
                    HtmlGenericControl sub_child_menu = (HtmlGenericControl)Repeater2.Items[i].FindControl("sub_child_menu");
    ...
    private void AddSecondChildNode(Repeater Repeater3, string parentid, string table, string isfor, string value)
    {
        sql = "SELECT MenuID,text,('" + isfor + "'+'/') as root,(Case When ISNULL(Url,'')='' Then '' Else Url End ) as URL,ParentClassName,ChildClassName FROM Menueam ";
        sql = sql + " Inner join " + table + " mp on mp.Menu_Id=Menueam.MenuID";
        ...
        Repeater3.DataSource = oo.GridFill(sql);
        Repeater3.DataBind();
    }
    ...
    protected void RemoveChieldItems(string str)
    {
        for (int i = 0; i < Repeater1.Items.Count; i++)
        {
            ...
            for (int j = 0; j < Repeater2.Items.Count; j++)
            {
                ...
                if (lblitem.Text.ToUpper() != "Marks Entry".ToUpper())
                {
                    Repeater Repeater3 = (Repeater)Repeater2.Items[j].FindControl("Repeater3_ss");
