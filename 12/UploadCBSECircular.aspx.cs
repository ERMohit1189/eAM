...
public partial class website_UploadCBSECircular : System.Web.UI.Page
{
    ...
    {
       // sql = "Select ROW_NUMBER() OVER (ORDER BY Id ASC) AS SrNo ,ID,Titel,ISNULL(DocName,Path) as DocName from CbseCircular order by Id desc ";
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = "USP_CbseCircular";
    ...
    protected void Button1_Click(object sender, EventArgs e)
    {
        try
        {
            sql = "select id from CbseCircular where Titel='" + txtImageName.Text + "'";
            if (oo.Duplicate(sql))
            {
