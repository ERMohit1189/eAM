    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["Logintype"].ToString() == "Admin" || Session["Logintype"].ToString() == "Guardian")
            {
                ...
                _sql = _sql + " where cm.SessionName='" + Session["SessionName"] + "' and t1.SessionName='" + Session["SessionName"] + "'  and t1.BranchCode=" + Session["BranchCode"] + " and GroupId='G6' Order by CIDOrder";
            ...
            else
            {
                ...
                _sql = _sql + " and cm.id in(select ClassId from dt_ClassGroupMaster where GroupId='G6' and SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + ")";
                            ...
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (drpEval.SelectedValue.ToUpper() == "TERM1")
                                {
                                    ...
                                    for (int m = 0; m < rptmarksT1.Items.Count; m++)
                                    {
                                        ...
                                        dts.Rows.Add(sname, highestMarks.ToString("0"), ObtMarks1.ToString("0"), (avgMarks / int.Parse(_oo.ReturnTag(totalStu, "totalStu"))).ToString("0"));
                                    }
                                ...
                                if (drpEval.SelectedValue.ToUpper() == "TERM2")
                                {
                                    ...
                                    for (int p = 0; p < ds.Tables[0].Rows.Count; p++)
                                    {
                                        ...
                                        dts.Rows.Add(sname, highestMarks.ToString("0"), totalObt.ToString("0"), (avgMarks2 / int.Parse(_oo.ReturnTag(totalStu, "totalStu"))).ToString("0"));
                                    }
                                    if (ds.Tables[1].Rows.Count > 0)
                                    {
                                        ...
                                        for (int p = 0; p < ds.Tables[1].Rows.Count; p++)
                                        {
                                            ...
                                            dts.Rows.Add(sname, (highestMarks / 2).ToString("0"), totalObt.ToString("0"), (avgMarks2 / int.Parse(_oo.ReturnTag(totalStu, "totalStu"))).ToString("0"));
