...
public partial class AditionalGradeEntry : System.Web.UI.Page
{
    ...
    public void loadclass()
    {
        ...
        else
        {
            ...
            sql +=  " and cm.id in(select ClassId from dt_ClassGroupMaster where GroupId='G6' and SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + ")";
    ...
    {
        sql = "select * from (   ";
        sql +=  "  select isnull(g.SubjectId,0) SubjectId, isnull(g.PaperId,0) PaperId, g.id, asr.SrNo, Name, FatherName,UT1, UT2,UT3, HY  from AllStudentRecord_UDF('" + Session["SessionName"].ToString() + "', " + Session["BranchCode"].ToString() + ") asr    ";
        sql +=  "  left join UPEAditionalEntryXI g on g.ClassId=asr.ClassId and g.BranchId=asr.BranchId and g.SectionId=asr.SectionID and g.BranchCode=asr.BranchCode and g.SessionName=asr.SessionName and g.SrNo=asr.SrNo and SubjectId=" + drpSubject.SelectedValue + " and PaperId=" + drpPaper.SelectedValue + "";
        ...
        sql +=  "  union all ";
        sql += "  select isnull(g.SubjectId,0) SubjectId,isnull(g.PaperId,0) PaperId, g.id, asr.SrNo, Name, FatherName,UT4 as UT1, UT5 as UT2,UT6 as UT3, AE as HY  from AllStudentRecord_UDF('" + Session["SessionName"].ToString() + "', " + Session["BranchCode"].ToString() + ") asr ";
        sql +=  "  left join UPEAditionalEntryXI g on g.ClassId=asr.ClassId and g.BranchId=asr.BranchId and g.SectionId=asr.SectionID and g.BranchCode=asr.BranchCode and g.SessionName=asr.SessionName and g.SrNo=asr.SrNo  and SubjectId=" + drpSubject.SelectedValue + " and PaperId=" + drpPaper.SelectedValue + "";
    ...
    protected void lnkSubmit_Click(object sender, EventArgs e)
    {
        if (GridView1.Rows.Count > 0)
        {
            ...
            foreach (GridViewRow gvr in GridView1.Rows)
            {
                ...
                cmd = new SqlCommand();
                cmd.CommandText = "sp_UPEAditionalEntryXI";
