...
public partial class common_G2_ReportCard_NurtoPrep_1718 : Page
{
    ...
    public void LoadReportCard()
    {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            {
                cmd.Connection = conn;
                cmd.CommandText = "ReportCard_NurtoPrep_2021";
                        ...
                        try
                        {
                            ...
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (drpEval.SelectedValue.ToUpper() == "TERM1")
                                {
                                    ...
                                    {
                                        double Test1 = 0; double Test2 = 0; double NB = 0; double SE = 0; double SAT = 0;
                                        ...
                                        double totalmarks1 = (((Test1 > Test2 ? Test1 : Test2) * 10) / 20) + NB + SE;
                                        double ObtMarks1 = (double.Parse(totalmarks1.ToString("0")) + double.Parse(SAT.ToString("0")));
                                        ...
                                        hi = hi + " else  isnull(TRY_CONVERT(decimal(10,0),Test2),0) end)*10)/20+isnull(TRY_CONVERT(decimal(10,0),NB),0)+isnull(TRY_CONVERT(decimal(10,0),SE),0) +isnull(TRY_CONVERT(decimal(10,0),SAT),0)) highestMarks ";
                                        hi = hi + " from CCENurtoPrep_1718 where ClassId=" + drpclass.SelectedValue + " and SectionName='" + drpsection.SelectedItem.Text + "' and Evaluation='TERM1' and SubjectId=" + ds.Tables[0].Rows[m]["SubjectId_1"].ToString() + "  ";
                                        ...
                                        avg = avg + " then isnull(TRY_CONVERT(decimal(10,0),Test1),0) else  isnull(TRY_CONVERT(decimal(10,0),Test2),0) end)*10)/20+isnull(TRY_CONVERT(decimal(10,0),NB),0)+isnull(TRY_CONVERT(decimal(10,0),SE),0)+isnull(TRY_CONVERT(decimal(10,0),SAT),0)) avgMarks";
                                        avg = avg + " from CCENurtoPrep_1718 where ClassId=" + drpclass.SelectedValue + " and SectionName='" + drpsection.SelectedItem.Text + "' and Evaluation='TERM1' and SubjectId=" + ds.Tables[0].Rows[m]["SubjectId_1"].ToString() + "  ";
                                ...
                                if (drpEval.SelectedValue.ToUpper() == "TERM2")
                                {
                                    ...
                                    for (int p = 0; p < ds.Tables[0].Rows.Count; p++)
                                    {
                                        ...
                                        double Test1_2 = 0; double Test2_2 = 0; double NB_2 = 0; double SE_2 = 0; double SAT_2 = 0;
                                        ...
                                        double totalmarks2 = (((Test1_2 > Test2_2 ? Test1_2 : Test2_2) * 10) / 20) + NB_2 + SE_2;
                                        double ObtMarks2 = (double.Parse(totalmarks2.ToString("0")) + double.Parse(SAT_2.ToString("0")));
                                        ...
                                        lblTheory2.Text = (SAT_2 == 0 ? "NP" : SAT_2.ToString("0"));
                                        ...
                                        hi = hi + " else  isnull(TRY_CONVERT(decimal(10,0),Test2),0) end)*10)/20+isnull(TRY_CONVERT(decimal(10,0),NB),0)+isnull(TRY_CONVERT(decimal(10,0),SE),0) +isnull(TRY_CONVERT(decimal(10,0),SAT),0))) highestMarks ";
                                        hi = hi + " from CCENurtoPrep_1718 where ClassId=" + drpclass.SelectedValue + " and SectionName='" + drpsection.SelectedItem.Text + "' and SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                        hi = hi + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "'";
                                        hi = hi + " and SrNo in (select SrNo from AllStudentRecord_UDF('" + Session["SessionName"] + "'," + Session["BranchCode"] + ") stu ";
                                        hi = hi + " where stu.sessionName='" + Session["SessionName"] + "' and isnull(stu.Promotion, '')<>'Cancelled' ";
                                        hi = hi + " and stu.ClassId=" + drpclass.SelectedValue + " and stu.SectionName='" + drpsection.SelectedItem.Text + "' ";
                                        hi = hi + " and isnull(Withdrwal,'') = '' ";
                                        hi = hi + " and isnull(blocked,'') = '')  group by SrNo ";
                                        hi = hi + "  )T1 --5order by T1.highestMarks desc ";
                                        double highestMarks = double.Parse((_oo.ReturnTag(hi, "highestMarks").ToString() == "" ? "0" : _oo.ReturnTag(hi, "highestMarks")));
                                        string avg = "select sum(T1.avgMarks)avgMarks from (select TRY_CONVERT(decimal(10,0),(TRY_CONVERT(decimal(10,0), (((case when isnull(TRY_CONVERT(decimal(10,0),Test1),0)> isnull(TRY_CONVERT(decimal(10,0),Test2),0)  then isnull(TRY_CONVERT(decimal(10,0),Test1),0) else  isnull(TRY_CONVERT(decimal(10,0),Test2),0) end)*10)/20)+isnull(TRY_CONVERT(decimal(10,0),NB),0)+isnull(TRY_CONVERT(decimal(10,0),SE),0) +isnull(TRY_CONVERT(decimal(10,0),SAT),0))/2)) avgMarks";
                                        avg = avg + " from CCENurtoPrep_1718 where ClassId=" + drpclass.SelectedValue + " and SectionName='" + drpsection.SelectedItem.Text + "' and SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                        avg = avg + " and BranchCode=" + Session["BranchCode"] + " and SessionName='" + Session["SessionName"] + "' ";
                                        avg = avg + " and SrNo in (select SrNo from AllStudentRecord_UDF('" + Session["SessionName"] + "'," + Session["BranchCode"] + ") stu ";
                                        avg = avg + " where stu.sessionName='" + Session["SessionName"] + "' and isnull(stu.Promotion, '')<>'Cancelled' ";
                                ...
                                rm += "where srm.BranchCode=" + Session["BranchCode"] + " and srm.SessionName='" + Session["SessionName"] + "' and SrNo='" + dt.Rows[i]["admissionNo"].ToString().Trim() + "' ";
