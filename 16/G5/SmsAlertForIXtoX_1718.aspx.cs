...
public partial class common_SmsAlertFroIXtoX_1718 : Page
{
    ...
    {
        sql = "Select Month,Convert(Varchar(11),id,106) StartingDate From(Select Distinct Left(DateName(MONTH, StartingDate), 3) Month,StartingDate Id from MonthMaster Where SessionName = '" + Session["SessionName"].ToString() + "' and BranchCode=" + Session["BranchCode"] + ")T1 order by Id Asc";
    ...
    public void loadclass()
    {
        ...
        else
        {
            if (Session["SessionName"].ToString() == "2015-2016")
            {
                ...
                sql +=  " where GroupId='G5' and cm.SessionName='" + Session["SessionName"] + "'  and t1.SessionName='" + Session["SessionName"] + "' and sctm.SessionName='" + Session["SessionName"] + "' and Ecode='" + Session["LoginName"].ToString() + "' and ClassTeacher='Yes' Order by CIDOrder";
                BAL.objBal.FillDropDown_withValue(sql, drpclass, "ClassName", "Id");
            }
            else
            {
    ...
    public void loadSubjects()
    {
        ...
        sql +=  " where sm.ClassId = '" + drpclass.SelectedValue.ToString() + "'  and sgm.SessionName='" + Session["SessionName"] + "' and spm.BranchCode=" + Session["BranchCode"] + " and sm.BranchCode=" + Session["BranchCode"] + " and sgm.BranchCode=" + Session["BranchCode"] + "  and spm.SessionName='" + Session["SessionName"] + "' and sm.SectionName = '" + drpsection.SelectedItem.Text.ToString() + "' and sm.SessionName = '" + Session["SessionName"].ToString() + "'";
    ...
    public void loadStudents()
    {
        ...
        sql +=  "   where sc.SectionName='" + drpsection.SelectedItem.Text + "' and cm.BranchCode=" + Session["BranchCode"] + " and sf.BranchCode=" + Session["BranchCode"] + " and sc.BranchCode=" + Session["BranchCode"] + " and so.BranchCode=" + Session["BranchCode"] + " and  sg.SessionName='" + Session["SessionName"].ToString() + "' and ";
        ...
        for (int i = 0; i < rptStudents.Items.Count; i++)
        {
            ...
            sql +=  " where sm.ClassId = '" + drpclass.SelectedValue.ToString() + "' and sm.BranchCode=" + Session["BranchCode"] + " and sgm.BranchCode=" + Session["BranchCode"] + " and sm.SectionName = '" + drpsection.SelectedItem.Text.ToString() + "' and sm.SessionName = '" + Session["SessionName"].ToString() + "'";
            sql +=  " and sgm.IsForOnlyExam = 1 and spm.IsForExam = 1 order by sgm.DisplayOrder Asc";
    ...
    public string GetMMarks(string Eval, string subjectid)
    {
        ...
        ds = DLL.objDll.Sp_SelectRecord_usingExecuteDataset("USP_CCEIXtoXMMMarksForMessage", param);
    ...
    public string GetTestMarks(string Eval, string subjectid,string srno)
    {
        ...
        ds = DLL.objDll.Sp_SelectRecord_usingExecuteDataset("USP_CCEIXtoXMarksForMessage", param);
