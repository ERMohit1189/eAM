...
public partial class ClassWiseStudentsMarksList : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                Response.Redirect("Studentwisecumulative_IXtoX.aspx?check=ClassWiseStudentsMarksList_IXtoX");
            }
            loadclass();
            loadsection();
            loadgrid();
    ...
    public void loadgrid()
    {
        ...
        sql +=  "   and SO.Withdrwal is null and SO.AdmissionForClassId='" + drpClass.SelectedValue.ToString() + "' and SO.SectionId='" + drpsection.SelectedValue.ToString() + "' order by SG.FirstName";
    ...
    public void loadsection()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                sql +=  " where EmpCode='" + Session["LoginName"].ToString() + "'  and sm.BranchCode=" + Session["BranchCode"] + " and t1.BranchCode=" + Session["BranchCode"] + " and IsClassTeacher=1 and T1.SessionName='" + Session["SessionName"].ToString() + "'";
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        if (RadioButtonList1.SelectedIndex == 0)
        {
            ...
            Response.Write("var winpop=window.open('ReportCardofIXtoX.aspx?print=1','_blank');  if(!winpop || winpop.closed){alert('Please allow pop-up blocker from browser settings!')}");
            Response.Write("</script>");
        }
        else if (RadioButtonList1.SelectedIndex == 1)
        {
            Response.Write("<script>");
            Response.Write("var winpop=window.open('ReportCardofIXtoXPer.aspx?print=1','_blank');  if(!winpop || winpop.closed){alert('Please allow pop-up blocker from browser settings!')}");
            Response.Write("</script>");
        }
        else
        {
            Response.Write("<script>");
            Response.Write("var winpop=window.open('ParticularStudentMarks_IXtoX.aspx?print=1','_blank');  if(!winpop || winpop.closed){alert('Please allow pop-up blocker from browser settings!')}");
            Response.Write("</script>");
        }
