...
public partial class common_MarksEntryReportVItoVIII_1718 : Page
{
    ...
    public void loadgrid()
    {
        ...
        sql = sql + "   and SC.SectionName='" + drpsection.SelectedItem.ToString() + "' and cm.BranchCode=" + Session["BranchCode"] + " and sc.BranchCode=" + Session["BranchCode"] + " and so.BranchCode=" + Session["BranchCode"] + " and sf.BranchCode=" + Session["BranchCode"] + " and sg.BranchCode=" + Session["BranchCode"] + " and sg.SessionName='" + Session["SessionName"].ToString() + "' and ";
        ...
        sql = sql + "   and SO.Withdrwal is null and  isnull(SO.Promotion, '')<>'Cancelled' order by FirstName Asc";
    ...
    public void loadclass()
    {
        if (Session["Logintype"].ToString() == "Admin")
        {
            ...
            sql = sql + " where cm.SessionName='" + Session["SessionName"] + "' and t1.BranchCode=" + Session["BranchCode"] + " and cm.BranchCode=" + Session["BranchCode"] + " and t1.SessionName='" + Session["SessionName"] + "' and GroupId='G4' Order by CIDOrder";
    ...
    public void loadsection()
    {
        ...
        else
        {
            ...
            sql = sql + " inner join SectionMaster sm on sm.ClassNameId=sctm.ClassId and sm.id=sctm.SectionId ";
            sql = sql + " and sm.SectionName=sctm.SectionName";
            sql = sql + " where sctm.BranchCode=" + Session["BranchCode"].ToString() + " and sm.BranchCode=" + Session["BranchCode"] + " and sctm.BranchCode=" + Session["BranchCode"] + " and sctm.SessionName='" + Session["SessionName"].ToString() + "' and sctm.ClassId='" + drpclass.SelectedValue.ToString() + "' and Ecode='" + Session["LoginName"].ToString() + "'";
            oo.FillDropDown_withValue(sql, drpsection, "SectionName", "Id");
            drpsection.Items.Insert(0, new ListItem("<--Select-->", "0"));
        }
    }
    ...
    public void loadSubject()
    {
        ...
        else
        {
            ...
            sql = sql + " inner join TTSubjectMaster sm on sm.Id=sctm.Subjectid and sm.ClassId=sctm.ClassId and sm.BranchId=sctm.BranchId and sm.SessionName=sctm.SessionName  and sm.BranchCode=sctm.BranchCode";
            sql = sql + " inner join ttpapermaster pm on pm.Id=sm.Subjectid and pm.ClassId=sctm.ClassId and pm.BranchId=sctm.BranchId and pm.SessionName=sctm.SessionName  and pm.BranchCode=sctm.BranchCode";
            sql = sql + " where Ecode='" + Session["LoginName"].ToString() + "'  and ApplicableFor<>'TimeTable' and sctm.ClassId=" + drpclass.SelectedValue.ToString() + " and pm.SubjectId='" + drpSubjectGroup.SelectedValue.ToString() + " ";
            sql = sql + " and  sctm.BranchCode = " + Session["BranchCode"] + " and sctm.SessionName='" + Session["SessionName"].ToString() + "'";
    ...
    private void loadSubjectMaxMarks(GridView grd, string Eval, string mmt1, string mmt2, string mmnb, string mmse, string mmfinal, string mmPrac)
    {
        sql = "Select MaxMarks1,MaxMarks2,MaxMarks4,MaxMarks5,MaxMarks6,MaxMarks7 from SetMaxMinMarks_VItoVIII where Eval='" + Eval + "' and BranchCode=" + Session["BranchCode"] + " and paperid='" + drpSubject.SelectedValue.ToString().Trim() + "' and SessionName='" + Session["SessionName"].ToString() + "'";
    ...
    protected void drpSubject_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (drpEval.SelectedIndex != 0)
            {
                ...
                loadMarks(GridView1, drpEval.SelectedValue.ToString(), "lblT1", "lblT2", "lblNB", "lblSE", "lblHY", "lblPrac", "Label2", "Label3", "lblTotal", "lblGrade", "lblMMT1", "lblMMT2", "lblMMNB", "lblMMSE", "lblMMHY", "lblMMPrac");
            }
            else
            {
                GridView1.Visible = false;
                GridView2.Visible = true;
                loadMarks(GridView2, "TERM1", "lblT1", "lblT2", "lblNB", "lblSE", "lblHY", "lblPrac", "Label2", "Label3", "lblTotal", "lblGrade", "lblMMT1", "lblMMT2", "lblMMNB", "lblMMSE", "lblMMHY", "lblMMPrac");
                loadMarks(GridView2, "TERM2", "lblT4", "lblT5", "lblNB2", "lblSE2", "lblHY2", "lblPrac2", "lblBestofTwo", "lblConinten", "lblTotal2", "lblGrade2", "lblMMT4", "lblMMT5", "lblMMNB2", "lblMMSE2", "lblMMHY2", "lblMMPrac2");
            }
        }
    }
    protected void loadMarks(GridView grd, string Eval, string test1, string test2, string nb, string se, string final, string prac, string total, string tenper, string finaltotal, string grademarks, string mmt1, string mmt2, string mmnb, string mmse, string mmfinal, string mmPrac)
    {
        if (grd.Rows.Count > 0)
        {
            loadSubjectMaxMarks(grd, Eval, mmt1, mmt2, mmnb, mmse, mmfinal, mmPrac);
            foreach (GridViewRow gvr in grd.Rows)
            {
                ...
                Label lblPer = (Label)gvr.FindControl(tenper);
                Label lblGrade = (Label)gvr.FindControl(grademarks);

                sql = "Select Test1,Test2,SAT,NB,SE, Prac from CCEVItoVIII_1718 where SRNO='" + lblsrno.Text + "' and BranchCode=" + Session["BranchCode"] + " and Evaluation='" + Eval + "' and paperid='" + drpSubject.SelectedValue.ToString() + "'";
            ...
            refresh(grd, Eval, test1, test2, nb, se, final, prac, mmt1, mmt2, mmnb, mmse, mmfinal, mmPrac, total, tenper, finaltotal, grademarks);
            createTitle();
        }

    }
    private void refresh(GridView grd, string Eval, string test1, string test2, string nb, string se, string final, string prac, string mmt1, string mmt2, string mmnb, string mmse, string mmfinal, string mmPrac, string total, string tenper, string finaltotal, string grademarks)
    {
        if (grd.Rows.Count > 0)
        {
            Label lblMMT1 = (Label)grd.HeaderRow.FindControl(mmt1);
            ...
            for (int i = 0; i < grd.Rows.Count; i++)
            {
                ...
                int mm4 = 0; int mm5 = 0; int mm6 = 0; int mm7 = 0;
                ...
                Label lblPrac = (Label)grd.Rows[i].FindControl(prac);
                ...
                else if (lblPrac.Text != "")
                {
                    result = double.TryParse(lblPrac.Text.Trim(), out num7);
                    if (result == true)
                    {
                        num7 = Convert.ToDouble(lblPrac.Text.Trim());
                    }
                    else
                    {
                        num7 = 0;
                ...
                else
                {
                    ...
                    Label2.Text = (totalmarks).ToString(CultureInfo.CurrentCulture);
                    ...
                    else
                    {
                        ...
                        Label3.Text = (Math.Round(percentle, 1)).ToString(CultureInfo.CurrentCulture);
                    }
                ...
                else
                {
                    ...
                    percentle = double.Parse((Math.Round(double.Parse(ptm) + num4 + num5 + num6 + num7)).ToString(CultureInfo.CurrentCulture));
    ...
    private void hideColumn()
    {
        ...
        ss = ss + " where cm.id=" + drpclass.SelectedValue + " and cm.BranchCode=" + Session["BranchCode"] + " and cm.SessionName='" + Session["SessionName"] + "'";
        ...
        if (drpEval.SelectedIndex != 0)
        {
            ...
            {
                Label lblHY = (Label)GridView1.HeaderRow.FindControl("lblHY1");
                Label lblPrac = (Label)GridView1.HeaderRow.FindControl("lblPrac1");
    ...
    {
        oo.ExporttolandscapePdf(Response, "SubjectwiseCumlativeVVItoVIIIIII", table1);
    }
    protected void ImageButton1_Click(object sender, EventArgs e)
    {
        oo.ExportTolandscapeWord(Response, "SubjectwiseCumlativeVVItoVIIIIII", table1);
    }
    protected void ImageButton2_Click(object sender, EventArgs e)
    {
        GridView1.Style.Add("text-transform", "uppercase");
        oo.ExportDivToExcelWithFormatting(Response, "SubjectwiseCumlativeVVItoVIIIIII.xls", table1, Server.MapPath("~/Admin/css/style.css"));
    }
