...
public partial class ddlSrNo : System.Web.UI.Page
{
...

    {
        ...
        _sql = "Select Name+' - '+SrNo NAME,SrNo from AllStudentRecord_UDF('" + Session["SessionName"].ToString() + "', "+Session["BranchCode"] + ") where ClassId=" + drpclass + " "; 
        ...
        _sql += "and isnull(blocked,'') = (case when '" + drpStatus + "'='W' or '" + drpStatus + "'='' then isnull(blocked,'') else case when '" + drpStatus + "'='A' then '' else 'yes' end end)";
