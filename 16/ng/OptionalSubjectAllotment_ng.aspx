<%@ Page Title="" Language="C#" MasterPageFile="~/11/comman_root_manager.master" AutoEventWireup="true" CodeFile="OptionalSubjectAllotment_ng.aspx.cs" Inherits="OptionalSubjectAllotment_ng" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolderMainBox" runat="Server">
    ...
    <script>
        ...
        function LoadSubject() {
            ...
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("Server/ddlOptionalSubject_MarkEntry.aspx") %>',
        ...
        function LoadFillList() {
            ...
            $.ajax({
                type: "POST",
                url: '<%=ResolveUrl("server/OptionalSubjectAllotmentServer.aspx") %>',
                data: {
                    'ClassId': ClassId,
                    'SectionId':SectionId,
                    'BranchId': BranchId,
        ...
        function SaveMarks() {
            ...
            var LoginName = '<%=HttpContext.Current.Session["LoginName"] %>';
                ...
                for (var i = 0; i < counts; i++) {
                    ...
                    if ($("[id*=divList] table tbody tr:eq(" + (i + 1) + ") td:eq(4) input[type=checkbox]").prop("checked") == true) {
                        status = "Checked";
                    }
                    else {
                        status = "Unchecked";
                ...
                $.ajax({
                    type: "POST",
                    url: '<%=ResolveUrl("Server/SaveOptionalSubjectAllotmentServer.aspx") %>',
                    data: {
                        'ClassId': ClassId,
                        'SubjectId': SubjectId,
                        'SessionName': SessionName,
