...
public partial class Comman_G7_PrintReportCardXII_1718_New : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["Logintype"].ToString() == "Admin" || Session["Logintype"].ToString() == "Guardian")
            {
                ...
                _sql = _sql + " where cm.SessionName='" + Session["SessionName"] + "' and t1.SessionName='" + Session["SessionName"] + "'  and t1.BranchCode=" + Session["BranchCode"] + " and GroupId='G7' Order by CIDOrder";
            ...
            else
            {
                ...
                _sql = _sql + " and cm.id in(select ClassId from dt_ClassGroupMaster where GroupId='G7' and SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + ")";
    ...
    public void LoadReportCard()
    {
        ...
        using (SqlConnection conn = new SqlConnection())
        {
            ...
            {
                cmd.Connection = conn;
                cmd.CommandText = "ReportCard_XII_2021";
                            ...
                            if (ds.Tables[0].Rows.Count > 0)
                            {
                                ...
                                if (drpEval.SelectedValue.ToUpper() == "TERM1")
                                {
                                    ...
                                    for (int m = 0; m < rptmarksT1.Items.Count; m++)
                                    {
                                        ...
                                        hi = hi + " select sum((((isnull(TRY_CONVERT(decimal(10,0),  (case when (isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)> ";
                                        hi = hi + " (isnull(TRY_CONVERT(decimal(10,0),m.Test2),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks2),1)  then (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20   ";
                                        hi = hi + " else  (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20 end)),0)  +isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)) ";
                                        hi = hi + " /(10+isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0))) ";
                                        hi = hi + " *isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0)) ";
                                        hi = hi + " +isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) highestMarks ";
                                        hi = hi + " from CCEXII_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                        hi = hi + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                        hi = hi + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.Evaluation='TERM1' and m.SubjectId=" + ds.Tables[0].Rows[m]["SubjectId_1"].ToString() + "  ";
                                        hi = hi + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' group by SrNo )T1";
                                    ...
                                    if (ds.Tables[1].Rows.Count > 0)
                                    {
                                        ...
                                        for (int m = 0; m < rptmarksT1Additional.Items.Count; m++)
                                        {
                                            ...
                                            hi = hi + " from CCEXII_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                            hi = hi + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                            hi = hi + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.Evaluation='TERM1' and m.SubjectId=" + ds.Tables[1].Rows[m]["SubjectId"].ToString() + "  ";
                                            hi = hi + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' group by SrNo )T1";
                                            double highestMarks = double.Parse((_oo.ReturnTag(hi, "highestMarks").ToString() == "" ? "0" : _oo.ReturnTag(hi, "highestMarks")));
                                        ...
                                        lblHTotal.Text = double.Parse(ds.Tables[7].Rows[0]["term1Total"].ToString()).ToString("0").Trim() + "/500";
                                        lblHPercentage.Text = ((double.Parse(double.Parse(ds.Tables[7].Rows[0]["term1Total"].ToString()).ToString("0")) * 100) / 500).ToString("0.00") + " %";
                                ...
                                if (drpEval.SelectedValue.ToUpper() == "TERM2")
                                {
                                    ...
                                    for (int p = 0; p < ds.Tables[0].Rows.Count; p++)
                                    {
                                        ...
                                        double MaxMarks1_2 = 0; double MaxMarks5_2 = 0; double MaxMarks3_2 = 0; double MaxMarks4_2 = 0; double Test1_2 = 0; double SAT2_2 = 0; double Prac_2 = 0; double SAT_2 = 0;
                                        ...
                                        if (ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "NP" || ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "NAD" || ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "ML" || ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "" || ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "AB" || ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "NA" || ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "M.L") { }
                                        else { MaxMarks5_2 = double.Parse(ds.Tables[0].Rows[p]["MaxMarks5_2"].ToString().Trim()); }
                                        ...
                                        double IA2 = 0;
                                        IA2 = (MaxMarks1_2 > 0 ? ((double.Parse(Test1_2.ToString("0")) * 10) / MaxMarks1_2) : 0);
                                        double bestSat = (SAT_2 > SAT2_2 ? double.Parse(SAT_2.ToString("0")) : double.Parse(SAT2_2.ToString("0")));
                                        double bestSatMM = (SAT_2 > SAT2_2 ? MaxMarks4_2 : MaxMarks5_2);
                                        double ConvinBoard_2 = (((double.Parse(IA2.ToString("0")) + double.Parse(bestSat.ToString("0"))) / (10 + bestSatMM)) * bestSatMM);
                                        ...
                                        lblAEMM.Text = bestSatMM.ToString("0");
                                        lblAE.Text = (bestSat == 0 ? "NP" : bestSat.ToString("0"));
                                        lblConvBoardMM2.Text = (((10 + bestSatMM) / (10 + bestSatMM)) * bestSatMM).ToString("0");
                                        lblConvBoard2.Text = ConvinBoard_2.ToString("0");
                                        lblPracMM2.Text = MaxMarks3_2.ToString("0");
                                        ...
                                        string hi = "select isnull(TRY_CONVERT(decimal(10,0),max(T1.highestMarks)),0) highestMarks  from ( ";
                                        hi = hi + " select isnull(sum(Ts.highestMarks1),0) highestMarks from ( ";
                                        hi = hi + " select sum((((isnull(TRY_CONVERT(decimal(10,0),  (case when (isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)> ";
                                        hi = hi + " (isnull(TRY_CONVERT(decimal(10,0),m.Test2),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks2),1)  then (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20   ";
                                        hi = hi + " else  (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20 end)),0)  +isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)) ";
                                        hi = hi + " /(10+isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0))) ";
                                        hi = hi + " *isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0)) ";
                                        hi = hi + " +isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) highestMarks1,m.SrNo ";
                                        hi = hi + " from CCEXII_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                        hi = hi + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                        hi = hi + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                        hi = hi + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' and m.Evaluation='TERM1' group by m.SrNo ";
                                        hi = hi + " union all ";
                                        hi = hi + " select sum((((((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*10)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)) +(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0),m.SAT),0) else isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) end)) ";
                                        hi = hi + " /(10+case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                        hi = hi + " *(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                        hi = hi + " +isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) highestMarks1,m.SrNo ";
                                        hi = hi + " from CCEXII_1718 m  ";
                                        hi = hi + " inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                        hi = hi + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName  ";
                                        hi = hi + " and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                        hi = hi + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                        hi = hi + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' and m.Evaluation='TERM2' group by m.SrNo ";
                                        hi = hi + " )Ts group by ts.SrNo";
                                        hi = hi + " )T1 ";
                                        double highestMarks = double.Parse((_oo.ReturnTag(hi, "highestMarks").ToString() == "" ? "0" : _oo.ReturnTag(hi, "highestMarks")));

                                        string avg = "select isnull(TRY_CONVERT(decimal(10,0),T1.avgMarks),0) avgMarks from ( ";
                                        avg = avg + " select isnull(sum(Ts.avgMarks1),0) avgMarks  from ( ";
                                        avg = avg + " select sum((((isnull(TRY_CONVERT(decimal(10,0),  (case when (isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)> ";
                                        avg = avg + " (isnull(TRY_CONVERT(decimal(10,0),m.Test2),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks2),1)  then (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20   ";
                                        avg = avg + " else  (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20 end)),0)  +isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)) ";
                                        avg = avg + " /(10+isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0))) ";
                                        avg = avg + " *isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0)) ";
                                        avg = avg + " + isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) avgMarks1 ";
                                        avg = avg + " from CCEXII_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                        avg = avg + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                        avg = avg + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                        avg = avg + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' and m.Evaluation='TERM1' ";
                                        avg = avg + " union all ";
                                        avg = avg + " select sum((((((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*10)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)) +(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0),m.SAT),0) else isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) end)) ";
                                        avg = avg + " /(10+case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                        avg = avg + " *(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                        avg = avg + " +isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) avgMarks1 ";
                                        avg = avg + " from CCEXII_1718 m  ";
                                        avg = avg + " inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                        avg = avg + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName  ";
                                        avg = avg + " and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                        avg = avg + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                        avg = avg + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' and m.Evaluation='TERM2' ";
                                        avg = avg + " )Ts ";
                                        avg = avg + " )T1 ";
                                    ...
                                    if (ds.Tables[1].Rows.Count > 0)
                                    {
                                        ...
                                        for (int p = 0; p < ds.Tables[1].Rows.Count; p++)
                                        {
                                            ...
                                            if (ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "NP" || ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "NAD" || ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "ML" || ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "" || ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "AB" || ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "NA" || ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim().ToUpper() == "M.L") { }
                                            else { MaxMarks5_2 = double.Parse(ds.Tables[1].Rows[p]["MaxMarks5_2"].ToString().Trim()); }
                                            ...
                                            string hi = "select isnull(TRY_CONVERT(decimal(10,0),max(T1.highestMarks)),0) highestMarks  from ( ";
                                            hi = hi + " select sum(Ts.highestMarks1)),0) highestMarks  from ( ";
                                            hi = hi + " select sum((((isnull(TRY_CONVERT(decimal(10,0),  (case when (isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)> ";
                                            hi = hi + " (isnull(TRY_CONVERT(decimal(10,0),m.Test2),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks2),1)  then (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20   ";
                                            hi = hi + " else  (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20 end)),0)  +isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)) ";
                                            hi = hi + " /(10+case when IsAditional=0 then isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) end)) ";
                                            hi = hi + " *case when IsAditional=0 then isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) end) ";
                                            hi = hi + " +isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) highestMarks1,m.SrNo ";
                                            hi = hi + " from CCEXII_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                            hi = hi + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                            hi = hi + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                            hi = hi + " and m.BranchCode=" + Session["BranchCode"] + " and m.SessionName='" + Session["SessionName"] + "' group by m.SrNo ";
                                            hi = hi + " union all ";
                                            hi = hi + " select sum((((((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*10)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)) +(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0),m.SAT),0) else isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) end)) ";
                                            hi = hi + " /(10+case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                            hi = hi + " *(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                            ...
                                            string avg = "select isnull(TRY_CONVERT(decimal(10,0),T1.avgMarks),0) avgMarks from ( ";
                                            hi = hi + " select sum(Ts.avgMarks1)),0) avgMarks  from ( ";
                                            avg = avg + " select sum((((isnull(TRY_CONVERT(decimal(10,0),  (case when (isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1)> ";
                                            avg = avg + " (isnull(TRY_CONVERT(decimal(10,0),m.Test2),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks2),1)  then (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20   ";
                                            avg = avg + " else  (((isnull(TRY_CONVERT(decimal(10,0),m.Test1),0)*20)/isnull(TRY_CONVERT(decimal(10,0),mm.MaxMarks1),1))*10)/20 end)),0)  +isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)) ";
                                            avg = avg + " /(10+case when IsAditional=0 then isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) end)) ";
                                            avg = avg + " *case when IsAditional=0 then isnull(TRY_CONVERT(decimal(10,0), MaxMarks3),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) end) ";
                                            avg = avg + " + isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) avgMarks1 ";
                                            avg = avg + " from CCEXII_1718 m inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                            avg = avg + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                            avg = avg + " where m.ClassId=" + drpclass.SelectedValue + " and m.SectionName='" + drpsection.SelectedItem.Text + "' and m.SubjectId=" + ds.Tables[0].Rows[p]["SubjectId_1"].ToString() + "  ";
                                            ...
                                            hi = hi + " /(10+case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                            hi = hi + " *(case when isnull(TRY_CONVERT(decimal(10,0),m.SAT),0)>isnull(TRY_CONVERT(decimal(10,0),m.SAT2),0) then isnull(TRY_CONVERT(decimal(10,0), MaxMarks4),0) else isnull(TRY_CONVERT(decimal(10,0), MaxMarks5),0) end)) ";
                                            hi = hi + " +isnull(TRY_CONVERT(decimal(10,0),m.Prac),0)) avgMarks1 ";
                                            hi = hi + " from CCEXII_1718 m  ";
                                            hi = hi + " inner join TTSubjectMaster sm on sm.Id=m.SubjectId and sm.Classid=m.ClassId and sm.SessionName=m.SessionName and sm.BranchCode=m.BranchCode ";
                                            hi = hi + " inner join SetMaxMinMarks_XII mm on mm.SubjectActivityId=m.SubjectId and m.PaperId=mm.PaperId and m.SessionName=mm.SessionName  ";
                                            hi = hi + " and m.BranchCode=mm.BranchCode and m.Evaluation=mm.Eval ";
                                        ...
                                        lblHTotal.Text = double.Parse(ds.Tables[7].Rows[0]["term1Total"].ToString()).ToString("0").Trim() + "/500";
                                        lblHTotal2.Text = double.Parse(ds.Tables[7].Rows[0]["term2Total"].ToString()).ToString("0").Trim() + "/500";
                                        ...
                                        var totalSum = results + results2;

                                        lblHTotalGT.Text = totalSum.ToString("0") + "/500";
                                        lblHPercentage.Text = ((double.Parse(double.Parse(ds.Tables[7].Rows[0]["term1Total"].ToString()).ToString("0")) * 100) / 500).ToString("0.00") + " %";
                                        lblHPercentage2.Text = ((double.Parse(double.Parse(ds.Tables[7].Rows[0]["term2Total"].ToString()).ToString("0")) * 100) / 500).ToString("0.00") + " %";
                                        lblHPercentageGT.Text = ((totalSum * 100) / 500).ToString("0.00") + " %";
    ...
    protected void drpsection_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            _sql = _sql + " inner join BranchMaster sm on sm.ClassId=sctm.ClassId and sm.Id=sctm.BranchId and sm.SessionName=sctm.SessionName and sm.BranchCode=sctm.BranchCode ";
            _sql = _sql + " where sctm.BranchCode=" + Session["BranchCode"].ToString() + " and sctm.BranchCode = " + Session["BranchCode"] + " and sctm.ClassId='" + drpclass.SelectedValue + "' and SectionId=" + drpsection.SelectedValue + " ";
            _sql = _sql + " and EmpCode='" + Session["LoginName"].ToString() + "' ";
            _oo.FillDropDown_withValue(_sql, drpBranch, "BranchName", "id");
        }
        drpBranch.Items.Insert(0, new ListItem("<--Select-->", ""));
