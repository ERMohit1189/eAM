...
public partial class admin_PlannerMaster : Page
{
    ...
    {
        BLL.BLLInstance.loadClass(drpFromClass, Session["SessionName"].ToString());
        drpFromClass.SelectedIndex = 1;

        BLL.BLLInstance.loadClass(drpToClass, Session["SessionName"].ToString());
        drpToClass.SelectedIndex = drpToClass.Items.Count - 1;
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        string classid = "";
        if (rbPlannerFor.SelectedIndex != 1)
        {
            for (int i = drpFromClass.SelectedIndex; i < drpToClass.SelectedIndex + 1; i++)
            {
                if (classid == string.Empty)
                {
                    classid = drpFromClass.Items[i].Value.ToString();
                }
                else
                {
                    classid = classid + "," + drpFromClass.Items[i].Value.ToString();
                }
            }
        }
        ...
        if (rbPlannerFor.SelectedIndex == 0)
        {
            param.Add(new SqlParameter("@StCountAtt", drpStudentCount.SelectedValue.ToString()));
        }
        else if (rbPlannerFor.SelectedIndex == 1)
        {
            param.Add(new SqlParameter("@StaffCountAtt", drpStaffCount.SelectedValue.ToString()));
        }
        else
        {
            param.Add(new SqlParameter("@StCountAtt", drpStudentCount.SelectedValue.ToString()));
    ...
    {     
        ...
        Grd.DataSource = DLL.objDll.Sp_SelectRecord_usingExecuteDataset("PlannerMasterProc", param);
        Grd.DataBind();        
    }
   ...
   public void PermissionGrant(int add1, int delete1, int update1, LinkButton Ladd, Button Ldelete, Button LUpdate)
    ...
    protected void rbPlannerFor_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (rbPlannerFor.SelectedIndex == 1)
        {
            //divfromClass.Visible = false;
            //divtoClass.Visible = false;
            divStudentCount.Visible = false;
            divStaffCount.Visible = true;
        }
        else if (rbPlannerFor.SelectedIndex == 0)
        {
            //divfromClass.Visible = true;
            //divtoClass.Visible = true;
            divStudentCount.Visible = true;
            divStaffCount.Visible = false;
        }
        else
        {
            //divfromClass.Visible = true;
