...
public partial class SuperAdmin_menu_accessability : Page
{
    ...
    public void Grd_Fill()
    {
        ...
        {
            sql = "select name+' ('+LoginName+')' LoginName, LoginId from LoginTab l left join NewAdminInformation na on na.UserId=l.LoginName where LoginTypeId='" + DropDownList2.SelectedValue.ToString() + "' and BranchId=" + Session["BranchCode"] + " and (IsActive is NULL or IsActive=1) ";
        ...
        {
            sql = "select (Name+' ('+Ecode+')') LoginName, LoginId, EmpId from GetAllStaffRecords_UDF(" + Session["BranchCode"] + ") asr inner join LoginTab lt on lt.LoginName=asr.Ecode and asr.BranchCode=lt.BranchId where lt.LoginTypeId='" + DropDownList2.SelectedValue.ToString() + "' and lt.BranchId=" + Session["BranchCode"] + " and (lt.IsActive is NULL or lt.IsActive=1) ";
            oo.FillDropDown_withValue(sql, DropDownList1, "LoginName", "LoginId");
        }
    ...
    public void CheckBoxCheck()
    {
        if (TreeView1.Nodes.Count > 0)
        {
                ...
                for (int i = 0; i < TreeView1.Nodes.Count; i++)
                {
                    sql = "Select Count(*) as Counter_Value  From menu_permission where Login_Id='" + DropDownList1.SelectedItem.Value + "' and Permission_Value='Yes' and Menu_Id='" + TreeView1.Nodes[i].Value + "' and BranchCode=" + Session["BranchCode"] + "";
                    ...
                    for (int j = 0; j < TreeView1.Nodes[i].ChildNodes.Count; j++)
                    {
                        sql = "Select Count(*) as Counter_Value  From menu_permission where Login_Id='" + DropDownList1.SelectedItem.Value + "' and Permission_Value='Yes' and Menu_Id='" + TreeView1.Nodes[i].ChildNodes[j].Value + "' and BranchCode=" + Session["BranchCode"] + "";
                        if (oo.ReturnTag(sql, "Counter_Value") == "1")
                        {
                        ...
                        for (int k = 0; k < TreeView1.Nodes[i].ChildNodes[j].ChildNodes.Count; k++)
                        {
                            sql = "Select Count(*) as Counter_Value  From menu_permission where Login_Id='" + DropDownList1.SelectedItem.Value + "' and Permission_Value='Yes' and Menu_Id='" + TreeView1.Nodes[i].ChildNodes[j].ChildNodes[k].Value + "' and BranchCode=" + Session["BranchCode"] + "";
                            if (oo.ReturnTag(sql, "Counter_Value") == "1")
                            {
    ...
    protected void Button1_Click(object sender, System.EventArgs e)
    {
        ...
        if (DropDownList1.SelectedItem.Text != "<--Select-->")
        {
            if (TreeView1.Nodes.Count > 0)
            {
                sql = "Select Count(*) as Counter From menu_permission Where Login_Id='" + DropDownList1.SelectedValue + "' and BranchCode=" + Session["BranchCode"] + "";
    ...
    private void loadUserName()
    {
        ...
        if (DropDownList2.SelectedIndex == 1)
        {
            sql = "Select top(1) (EFirstName+case when EMiddleName=' ' then ' ' else ' '+EMiddleName+' ' End+ELastName) EmpName  from EmpGeneralDetail Where Ecode = '" + DropDownList1.SelectedItem.Text.ToString() + "' and BranchCode=1 Order by SrNo desc";
        ...
        else
        {
            ...
            sql = sql + " and BranchId=" + Session["BranchCode"] + " and Password=(select Pass from LoginTab where LoginId='" + DropDownList1.SelectedValue.ToString() + "')";
