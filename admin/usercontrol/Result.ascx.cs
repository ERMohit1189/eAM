...
public partial class admin_Examination : System.Web.UI.UserControl
{
        ...
        if (ds.Tables.Count > 0)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["collegeLogo"].ToString() == "")
                {
                    ...
                    Image3.ImageUrl = "~/uploads/CollegeLogo/DefaultCollegeLogo.png";

                }
                else
                {
                    Image1.ImageUrl = BAL.objBal.GetImageUrl(ResolveClientUrl(dt.Rows[0]["collegeLogo"].ToString()), Request.Url.AbsoluteUri.Split('/'));
                    Image3.ImageUrl = BAL.objBal.GetImageUrl(ResolveClientUrl(dt.Rows[0]["collegeLogo"].ToString()), Request.Url.AbsoluteUri.Split('/'));
                }
                ...
                string MarginTop = "", MarginBottom = "", MarginLeft = "", MarginRight = "";
                string sql = "select  MarginTop, MarginBottom, MarginLeft, MarginRight from DocSetting where DocCategory='Result' and BranchCode=" + Session["BranchCode"] + "";
                if (oo.Duplicate(sql))
                {

                    MarginTop = (double.Parse(oo.ReturnTag(sql, "MarginTop")) * 38).ToString();
                    MarginBottom = (double.Parse(oo.ReturnTag(sql, "MarginBottom")) * 38).ToString();
                    MarginLeft = (double.Parse(oo.ReturnTag(sql, "MarginLeft")) * 38).ToString();
                    MarginRight = (double.Parse(oo.ReturnTag(sql, "MarginRight")) * 38).ToString();
                }


                ctrl13.Style.Add("margin-top", MarginTop + "px");
                ctrlmain.Style.Add("margin-top", MarginTop + "px");
                ctrl1.Style.Add("margin-top", MarginTop + "px");
                ctrl13.Style.Add("margin-bottom", MarginBottom + "px");
                ctrlmain.Style.Add("margin-bottom", MarginBottom + "px");
                ctrl1.Style.Add("margin-bottom", MarginBottom + "px");
                ctrl13.Style.Add("margin-left", MarginLeft + "px");
                ctrl1.Style.Add("margin-right", MarginRight + "px");
            }
        }
    }
