...
public partial class admin_Receipt : System.Web.UI.UserControl
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (branchCode == "" && Request.QueryString.Count > 0)
        {
            string[] arr = Request.QueryString["RecieptSrNo"].ToString().Split('$');
            branchCode = arr[2].ToString();
        }
        ...
        if (ds.Tables.Count > 0)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    Image3.ImageUrl = logo;// BAL.objBal.GetImageUrl(ResolveClientUrl(dt.Rows[0]["collegeLogo"].ToString()), Request.Url.AbsoluteUri.Split('/'));
                }
