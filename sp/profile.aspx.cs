...
{
    string sql = "";
    string branchcode, table = string.Empty;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                sql = "Select LoginName,Pass from LoginTab where LoginId='" + Session["LoginId"].ToString() + "' and BranchId = " + Session["BranchCode"] + " and (IsActive=1 or IsActive is null)";
                ...
                sql = sql + " where UserId='" + loginname + "' and nai.BranchCode = " + Session["BranchCode"] + " and Password='" + pass + "' " + branchcode + "";
                ...
                {
                    imgAvatars.ImageUrl = "~/img/user-pic/user-pic.jpg";
                }
                try
                {
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        if (avatarUpload.HasFile)
        {
            ...
            try
            {
                ...
                sql = sql + " where " + matchingcolumn + "='" + loginname + "' " + password + " and BranchCode = " + Session["BranchCode"] + "";
            ...
            if (count > 0)
            {
                BAL.objBal.MessageBoxforUpdatePanel("Photo uploaded Successfully.", LinkButton1);

                ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect", "window.location='" + Request.ApplicationPath + "/profile.aspx';", true);
            }
            else
            {
                BAL.objBal.MessageBoxforUpdatePanel(msg, LinkButton1);
            }
        }
        else
        {
    ...
    protected void LinkButton2_Click(object sender, EventArgs e)
    {
        ...
        if (count > 0)
        {
            BAL.objBal.MessageBoxforUpdatePanel("Photo deleted Successfully.", LinkButton2);

            ScriptManager.RegisterStartupScript(this, this.GetType(), "redirect", "window.location='" + Request.ApplicationPath + "/profile.aspx';", true);
        }
        else
        {
            BAL.objBal.MessageBoxforUpdatePanel(msg, LinkButton2);
        }
