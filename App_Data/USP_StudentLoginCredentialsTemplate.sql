CREATE  Proc [dbo].[USP_StudentLoginCredentialsTemplate]
(
	@BranchCode int,
...
AS
BEGIN
	Select FirstName StudentName,LoginName Username,StudentPassword Password,IIF(ISNULL(MobileNumber,'')='',FatherContactNo,MobileNumber) ContactNo
	from AllStudentRecord_UDF(@SessionName,@BranchCode) cfd
	Inner Join LoginTab lt on lt.LoginName=cfd.SrNo
	...
	Where st.pagename='StudentLoginCredentials' and stp.BranchCode=@BranchCode
	Order by OrderNo
