...
public partial class HostelFeeReciept_duplicate : Page
{
    ...
    public void DisplayStudentCopy()
    {
        ...
        sql = "Select Top 1 DepositDate, Paid, LoginName  from HostelFeeDeposit where ReceiptNo='" + Label1.Text + "'";
        lblFooterDate0.Text = lblFooterDate.Text = DateTime.Parse(oo.ReturnTag(sql, "DepositDate")).ToString("dd MMM yyyy hh:mm tt");
    ...
    {
        sql = "select 'HOSTEL FEE' AS HeadName, Amount, '' AS remark from  HostelFeeDeposit where ReceiptNo='"+ Session["HostelReceiptNo"].ToString() + "'";
