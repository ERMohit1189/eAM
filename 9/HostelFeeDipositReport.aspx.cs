...
public partial class HostelFeeDipositReport : Page
    {
    ...
    protected void Bind(string txtSearch)
    {
        ...
        _sql = _sql + " case when '" + rdoType.SelectedValue + "' = 'Student' then(select CombineClassName from AllStudentRecord_UDF('" + drpSession.SelectedItem.Text + "', 1) where SrNo = SrNoOrEmpId) else 'Staff' end Class,";
        _sql = _sql + " * from HostelFeeDeposit";
        _sql = _sql + " where SrNoOrEmpid= case when '" + txtSearch + "'= '' then SrNoOrEmpid else '" + txtSearch + "' end and SessionName='" + Session["SessionName"].ToString() + "' and BranchCode=" + Session["BranchCode"].ToString() + " ";
