            ...
            if (dt.Rows.Count > 0)
            {
                using (SqlConnection conn = new SqlConnection())
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        {
                            stImage.ImageUrl = dsPhoto.Tables[0].Rows[0]["PhotoPath"].ToString() != string.Empty ? "../" + dsPhoto.Tables[0].Rows[0]["PhotoPath"] + "?value=" + DateTime.Now.ToString(CultureInfo.InvariantCulture) : "~/img/user-pic/user-pic.jpg" + "?value=" + DateTime.Now.ToString(CultureInfo.InvariantCulture);
                            stImage2.ImageUrl = stImage.ImageUrl;
                        }
                    }
                }
