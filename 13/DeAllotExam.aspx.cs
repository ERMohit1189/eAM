...
public partial class DeAllotExam : Page
{
    ...
    protected void LoadData()
    {
        ...
        _sql = _sql + "  and SRNo in (select SRNo from OT_ExamAllotment where SubjectId=" + ddlSubject.SelectedValue + " and PaperId=" + ddlPaper.SelectedValue + " and ExamID=" + ddlExam.SelectedValue + " and SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + ") and classid=" + ddlClass.SelectedValue+"";
        _sql = _sql + "  and SRNo not in (select SRNo from OT_ExamAnswerResult where classid=" + ddlClass.SelectedValue + " and SubjectId=" + ddlSubject.SelectedValue + " and PaperId=" + ddlPaper.SelectedValue + " and ExamID=" + ddlExam.SelectedValue + " and SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + ") ";
        ...
        for (int i = 0; i < Grd.Rows.Count; i++)
        {
            ...
            if (chk.Checked)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@Action", "deallot");
                    try
                    {
                        _con.Open();
                        cmd.ExecuteNonQuery();
