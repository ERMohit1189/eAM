        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (Request.QueryString.Keys.Count > 0)
                {
                    ...
                    {
                        cmd.CommandText = "update OT_ExamAllotment set Status=1, Duration=0 where ExamID=" + ExamID + " and  SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + " and SRNo='" + Session["LoginName"].ToString() + "'";
                ...
                {
                    _sql = "select ResultHide from OT_ExamMaster where getdate() between convert(datetime, isnull(ResultShow, '01-Jan-2050 10:00 AM')) and convert(datetime, isnull(ResultHide, '01-Jan-2050 10:50 AM')) and id=" + ExamID + "";
                    if (_oo.Duplicate(_sql))
                    {
                    }
                    else
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (dt.Rows[i]["QuestionType"].ToString() == "MCQs")
                {
                    ...
                    _sql = "select isnull(sum(ObtaindMarks), 0) ObtaindMarks from OT_ExamAnswerResult where ExamId = " + ExamID + " and SectionId=" + dt.Rows[i]["id"].ToString() + " and  SessionName='" + Session["SessionName"] + "' and BranchCode=" + Session["BranchCode"] + " and SrNO='" + Session["LoginName"].ToString() + "'";
