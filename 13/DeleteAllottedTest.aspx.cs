...
public partial class DeleteAllottedTest : Page
{
    private SqlConnection _con;
    private readonly Campus _oo;
    private string _sql, _sql1 = String.Empty;
    public DeleteAllottedTest()
    {
    ...
    protected void LoadData()
    {
        Grd.DataSource =null;
        Grd.DataBind();
        _sql = "Select distinct asr.SRNo, Name, FatherContactNo MobileNo, FatherName, DOB DateofBirth, CombineClassName from AllStudentRecord_UDF('" + Session["SessionName"].ToString() + "','" + Session["BranchCode"].ToString() + "') asr inner join OT_ExamAnswerResult ot on asr.SrNo = ot.SRNo  and asr.ClassId=ot.ClassId and asr.SectionID=ot.SectionId and  ot.SessionName='" + Session["SessionName"] + "' and ot.BranchCode=" + Session["BranchCode"] + " and SubjectId=" + ddlSubject.SelectedValue + " and PaperId=" + ddlPaper.SelectedValue + " and ExamID=" + ddlExam.SelectedValue + " where Withdrwal is null and isnull(blocked, '')='' ";
        _sql = _sql + "   and asr.classid="+ddlClass.SelectedValue+ " ";
        ...
        for (int i = 0; i < Grd.Rows.Count; i++)
        {
            ...
            if (chk.Checked)
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@Action", "deleteExam");
                    try
                    {
                        _con.Open();
                        cmd.ExecuteNonQuery();
