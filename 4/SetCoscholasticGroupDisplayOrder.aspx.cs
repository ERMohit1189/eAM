...
public partial class admin_temp_AddDisplayOrder : System.Web.UI.Page
{
    ...
    {
        sql = "Select CoscholasticGroup as CoscholasticGroup,Id From CoscholasticGroupMaster where Classid='" + drpclass.SelectedValue.ToString() + "' and SessionName='" + Session["SessionName"].ToString() + "' and BranchCode=" + Session["BranchCode"] + "";
        ...
        {
            //oo.MessageBoxforUpdatePanel("Sorry, No co-scholastic group created for this class!", this.Page);
            Campus camp = new Campus(); camp.msgbox(this.Page, msgbox, "Sorry, No co-scholastic group created for this class!", "A");       
    ...
    protected void LinkButton1_Click(object sender, EventArgs e)
    {
        ...
        if (flag)
        {
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                sql = "Update CoscholasticGroupMaster set DisplayOrder=" + drpOrder.SelectedValue + " where id='" + id.Text + "' and BranchCode=" + Session["BranchCode"] + "";
    ...
    {      
        if (GridView1.Rows.Count > 0)
        {
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                DropDownList drpDisplayNo = (DropDownList)GridView1.Rows[i].FindControl("drpDisplayNo");
                Label id = (Label)GridView1.Rows[i].FindControl("Label3");
                sql = "Select DisplayOrder From CoscholasticGroupMaster where id='" + id.Text + "' and BranchCode=" + Session["BranchCode"] + "";
